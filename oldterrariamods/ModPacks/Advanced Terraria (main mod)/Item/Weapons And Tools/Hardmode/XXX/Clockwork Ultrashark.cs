...
//Fire Gun (Finally)
		...
		if(secondBarrel) {
			Projectile.NewProjectile(p.position.X + (p.width * 0.5f) - normal.X + offset.X, p.position.Y + (p.height * 0.5f) - normal.Y + offset.Y, (float) speedX, (float) speedY, ammo.shoot, dmg, p.inventory[p.selectedItem].knockBack, playerID);
		}
		else
		{
			Projectile.NewProjectile(p.position.X + (p.width * 0.5f) + normal.X + offset.X, p.position.Y + (p.height * 0.5f) + normal.Y + offset.Y, (float) speedX, (float) speedY, ammo.shoot, dmg, p.inventory[p.selectedItem].knockBack, playerID);
