public void AI()
{
    ...
    {
        int num85 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y + (float)npc.height * 0.25f), npc.width, (int)((float)npc.height * 0.5f), 27, npc.velocity.X, 2f, 75, npc.color, npc.scale);
    ...
    {
        int num86 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y + (float)npc.height * 0.25f), npc.width, (int)((float)npc.height * 0.5f), 62, npc.velocity.X, 2f, 0, default(Color), 1f);
	...
	if (Main.netMode != 1 && !Main.player[npc.target].dead)
    {
        ...
        if (npc.localAI[0] == 180f)
        {
            ...
            {
                NPC.NewNPC((int)(npc.position.X + (float)(npc.width / 2) + npc.velocity.X), (int)(npc.position.Y + (float)(npc.height / 2) + npc.velocity.Y), Config.npcDefs.byName["Chaotic Orb"].type, 0);
            }
            npc.localAI[0] = 0f;
        }
    }
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= C; i++)
	{
		int D = Dust.NewDust(npc.position, R.Width, R.Height, 27, 0, 0, 100, new Color(), 2f);
	...
	{
		int D2 = Dust.NewDust(npc.position, R.Width, R.Height, 62, 0, 0, 100, new Color(), 1f);
