public void NPCLoot()
{
 Gore.NewGore(npc.position,npc.velocity,"Worg Head",1f,-1);
  Gore.NewGore(npc.position,npc.velocity,"Worg Arm",1f,-1);
   Gore.NewGore(npc.position,npc.velocity,"Worg Leg",1f,-1);
    Gore.NewGore(npc.position,npc.velocity,"Worg Leg",1f,-1);
}
public static bool SpawnNPC(int x, int y, int playerID) {
      ...
      if (!Main.player[playerID].zoneDungeon //not in dungeon. Note that the ! means "Not"
          ...
          && Main.rand.Next(70)==1){ //1 out of 8 chance of spawning
                 return true;
      }
   return false;
}