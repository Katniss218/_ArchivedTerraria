public void AI(){
{
	...
	npc.netUpdate = false;
	if(npc.justHit) {
		NPC.NewNPC((int)npc.position.X,(int)npc.position.Y,"Mecha Beholder",0);
	}
	npc.ai[1]++;
	if (npc.ai[1] >= 1600 && Main.netMode != 1) {
		int Wandererspawn = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Mecha Golem", 0);
    ...
    {
        int num85 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y + (float)npc.height * 0.25f), npc.width, (int)((float)npc.height * 0.5f), 54, npc.velocity.X, 2f, 75, npc.color, npc.scale);
    ...
    {
        int num86 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y + (float)npc.height * 0.25f), npc.width, (int)((float)npc.height * 0.5f), 1, npc.velocity.X, 2f, 0, default(Color), 1f);
	...
	if (Main.netMode != 1 && !Main.player[npc.target].dead)
    {
        ...
        if (npc.localAI[0] == 180f)
        {
            ...
            {
                NPC.NewNPC((int)(npc.position.X + (float)(npc.width / 2) + npc.velocity.X), (int)(npc.position.Y + (float)(npc.height / 2) + npc.velocity.Y), Config.npcDefs.byName["Hate"].type, 0);
            }
            npc.localAI[0] = 0f;
        }
    }
