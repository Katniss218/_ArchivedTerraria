public void AI()
{
	...
	Color color = new Color();
    	int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 64, npc.velocity.X, npc.velocity.Y, 200, color, 1f);
	...
	{
	Main.player[npc.target].AddBuff(Config.buffID["Curious Bat"], 600, false);
	}
