public void AI(){
{
	...
	npc.netUpdate = false;
	if(npc.justHit) {
		NPC.NewNPC((int)npc.position.X,(int)npc.position.Y,"Hatchling",0);
		}
...
public void DamagePlayer(Player P, ref int dmg)
{
	...
	{
		P.AddBuff("Sunder", 600, false);
	}
}

public void NPCLoot()
{
	Gore.NewGore(npc.position,npc.velocity,"Thunder Body",1f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Thunder Horn",1f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Thunder Horn",1f,-1);
