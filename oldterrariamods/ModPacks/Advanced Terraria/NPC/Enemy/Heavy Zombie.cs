...
{
	if (Main.player[playerID].townNPCs < 1 && y < (Main.maxTilesY - 200) && !Main.dayTime && !Main.player[playerID].zoneDungeon && !Main.player[playerID].zoneMeteor && Main.rand.Next(35) == 0)
	{
		return true;
	}
	else return false;
