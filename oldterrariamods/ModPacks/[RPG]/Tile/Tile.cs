public void ModifyWorld() {
        Main.statusText = "Adding Coal ...";
        for(int i=0;i<5+(int)(Main.maxTilesY/5);i++) AddOres();
        ...
        for(int x=0;x<Main.maxTilesX;x++) {
                for(int y=0;y<Main.maxTilesY;y++) {
                        ...
                        try {
                                ...
                                if(type==7 || type==8 || type==9 || type==Config.tileDefs.ID["Coal Ore"]) countType[type]++; 
                        } catch {
                        }
        ...
        data+="Greenium: "+countType[Config.tileDefs.ID["Coal Ore"]]+"\n";
...
public void AddOres() {
    ...
    WorldGen.OreRunner(i2, j2, (double)WorldGen.genRand.Next(5, 9 + add), WorldGen.genRand.Next(5, 9 + add), Config.tileDefs.ID["Coal Ore"]);
}