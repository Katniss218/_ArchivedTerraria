...
Lighting.addLight((int)((this.npc.position.X + (float)(this.npc.width / 2)) / 16f), (int)((this.npc.position.Y + (float)(this.npc.height / 2)) / 16f), 0.3f, 0.3f, 0.9f);  


#region alive check
if(!ModWorld.stationExists[2] || ModWorld.stationPosX[2] == -1 || ModWorld.stationPosY[2] == -1)
{
...
if(!Main.player[index].active || Main.player[index].dead || (!Main.player[index].hostile || (Main.player[index].team == 3 || Main.player[index].team == 0)))
{
this.npc.spriteDirection = 1;
...
if(index > -1)
    Main.NewText(""+Main.player[index].name+" Has Captured the Blue Flag!");
#endregion
}
#endregion
...
for (int i = 0; i < 4; i++)
{
if (i != 2 && i == Main.player[index].team-1 && ModWorld.stationExists[i])
    {
...
index = -1;
resetcooldown = 0;
this.npc.position.X = ModWorld.stationPosX[2]*16;
this.npc.position.Y = ModWorld.stationPosY[2]*16;
#region add this
if (this.npc.spriteDirection == -1)
...
public void Kill()
{
    if(ModWorld.stationExists[2])
    NPC.NewNPC((int)npc.position.X,(int)npc.position.Y,"Blue Flag",0);
}