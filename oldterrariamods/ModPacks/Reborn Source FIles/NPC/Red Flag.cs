...
Lighting.addLight((int)((this.npc.position.X + (float)(this.npc.width / 2)) / 16f), (int)((this.npc.position.Y + (float)(this.npc.height / 2)) / 16f), 0.9f, 0.3f, 0.3f);  


#region alive check
if(!ModWorld.stationExists[0] || ModWorld.stationPosX[0] == -1 || ModWorld.stationPosY[0] == -1)
{
...
if(!Main.player[index].active || Main.player[index].dead || (!Main.player[index].hostile || (Main.player[index].team == 1 || Main.player[index].team == 0)))
{
this.npc.spriteDirection = 1;
...
if(index > -1)
    Main.NewText(""+Main.player[index].name+" Has Captured the Red Flag!");
#endregion
}
#endregion
...
for (int i = 0; i < 4; i++)
{
if (i != 0 && i == Main.player[index].team-1 && ModWorld.stationExists[i])
    {
...
index = -1;
resetcooldown = 0;
this.npc.position.X = ModWorld.stationPosX[0]*16;
this.npc.position.Y = ModWorld.stationPosY[0]*16;
#region add this
if (this.npc.spriteDirection == -1)
...
public void Kill()
{
    if(ModWorld.stationExists[0])
    NPC.NewNPC((int)npc.position.X,(int)npc.position.Y,"Red Flag",0);
}