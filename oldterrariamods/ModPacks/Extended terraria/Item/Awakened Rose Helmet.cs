...
public void DamagePlayer(Player P, ref int damage, NPC N)
{
	...
	for (int k = 0; k < 10; k++)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			int num54 = Projectile.NewProjectile(Main.dust[dust[j]].position.X, Main.dust[dust[j]].position.Y, Main.dust[dust[j]].velocity.X, Main.dust[dust[j]].velocity.Y, Config.projDefs.byName["Leaves"].type, 25, 2, Main.myPlayer);
		}
	}
	for (int m = 0; m < 10; m++)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			int num54 = Projectile.NewProjectile(Main.dust[dust2[j]].position.X, Main.dust[dust2[j]].position.Y, Main.dust[dust2[j]].velocity.X, Main.dust[dust2[j]].velocity.Y, Config.projDefs.byName["Leaves"].type, 25, 2, Main.myPlayer);
		}
	}
}