...
public void AI() {
    ...
    if(CHARGE == 149)
    {
	Pindex = Projectile.NewProjectile(P.position.X + P.width / 2, P.position.Y + P.height / 2, P.velocity.X, P.velocity.Y, Config.projDefs.byName["The Sun"].type, (int)(O.inventory[O.selectedItem].damage * O.magicDamage), 3f, P.owner);
...
public void Kill() {
    ...
    /*foreach (Projectile P in Main.projectile) {
        if (P.active) {
            if (P.type == Config.projDefs.byName["The Sun"].type && P.owner == tP.owner)
                P.Kill();
        }
    }*/
    tP.active = false;
