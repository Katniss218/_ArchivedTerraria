...
public void AI() {
    ...
    if(CHARGE == 99) {
        Pindex = Projectile.NewProjectile(P.position.X + P.width / 2, P.position.Y + P.height / 2, P.velocity.X, P.velocity.Y, Config.projDefs.byName["Venoshock"].type, (int)50 + (O.rangedCrit / 2), 3f, P.owner);
...
public void Kill() {
    ...
    foreach (Projectile P in Main.projectile) {
        if (P.active) {
            if (P.type == Config.projDefs.byName["Venoshock"].type && P.owner == tP.owner)
                P.Kill();
        }
    }
    tP.active = false;
