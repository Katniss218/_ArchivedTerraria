public void AI()
{
	...
	if (P.ai[0] >= 150)
	{
		for (int i = 0; i < 3; i++)
		{
			...
			int a = Projectile.NewProjectile(P.position.X, P.position.Y, vX, vY, Config.projectileID["Smogscreen 4"], P.damage, P.knockBack, P.owner);
		}
		P.ai[0] = 0;
	}
}
public bool tileCollide(Vector2 CollideVel)
{
	...
	{
		if (projectile.type == Config.projDefs.byName["Smogscreen 3"].type && projectile.velocity.Y > 4f)
		{
