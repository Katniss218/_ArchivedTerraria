    ...
    if (P.ai[0] == 0f)
    {
        ...
        if (P.alpha <= 0)
        {
            ...
            if (P.type == 7 && Main.myPlayer == P.owner)
            {
                ...
                if (P.ai[1] >= 6) // change for max parts
                {
                    Projectile_Type = Config.projDefs.byName["Hallowed Thorn End"].type;
                }
                else
                    Projectile_Type = Config.projDefs.byName["Hallowed Thorn"].type;
                if((int)(P.ai[1])%3==0)
                {
