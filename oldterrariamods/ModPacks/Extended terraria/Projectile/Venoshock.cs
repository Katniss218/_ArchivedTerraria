...
bool DrawLazer = true;
int cooldown = 0;
public float Lr = 0.2275f, Lg = 0.8509f , Lb = 0.2588f;
...
public void AI() {
	...
	{
		npcIndex = ModWorld.FindClosestNPC(new Vector2((Main.mouseX + Main.screenPosition.X), (Main.mouseY + Main.screenPosition.Y)), 144); //P.position + new Vector2(L * P.velocity.X, L * P.velocity.Y);
...
public void PostDraw(SpriteBatch SB) {
    //if (projectile.active) {
        DrawChain(projectile.position, End, "beam2", SB);
    //}
}
