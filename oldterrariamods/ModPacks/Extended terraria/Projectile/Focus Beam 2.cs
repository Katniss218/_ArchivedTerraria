...
bool DrawLazer = true;
int cooldown = 0;
public float Lr = 0.2f, Lg = 0.3f , Lb = 0.8f;
public float aLr = 1f, aLg = 1f , aLb = 1f;
public float Ds = 2f;
public int DT = 15;
public Color DC = Color.Blue;
float dustSpeed = 4f;
...
public void AI() {
    ...
    if (P.active) {
        ...
        End = P.position + new Vector2(L * P.velocity.X, L * P.velocity.Y);
    }
	cooldown++;
	if (cooldown >= 4)
	{
...
public void PostDraw(SpriteBatch SB) {
    //if (projectile.active) {
        DrawChain(projectile.position, End, "beam", SB);
    //}
}
