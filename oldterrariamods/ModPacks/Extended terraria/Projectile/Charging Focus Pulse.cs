...
public void AI() {
    ...
    if(CHARGE == 59) {
        Pindex = Projectile.NewProjectile(P.position.X + P.width / 2, P.position.Y + P.height / 2, P.velocity.X, P.velocity.Y, Config.projDefs.byName["Focus Pulse"].type, (int)50 + (O.rangedCrit / 2), 3f, P.owner);
        Main.PlaySound(2, (int)P.position.X, (int)P.position.Y, SoundHandler.soundID["Pulse"]);
    }
    if(CHARGE > 61) CHARGE = 61;
...
public void Kill() {
    ...
    foreach (Projectile P in Main.projectile) {
        if (P.active) {
            if (P.type == Config.projDefs.byName["Focus Pulse"].type && P.owner == tP.owner)
                P.Kill();
        }
    }
    tP.active = false;
