public bool SpawnNPC(int x, int y, int playerID) 
{
	...
	{
		if (Main.player[playerID].townNPCs < 1 && Main.player[playerID].zoneEvil && !Main.player[playerID].zoneMeteor && y > (Main.maxTilesY - 275) && y < (Main.maxTilesY - 200) && Main.rand.Next(7) == 0)
		{
			return true;
		}
		else return false;
...
public void AI()
{
								...
								{
									if (npc.type == Config.npcDefs.byName["Armored Hell Corruptor"].type)
									{
										num73 = 4.2f;
										num74 = 0.022f;
									}
								...
								if (npc.type == 6 || npc.type == 42 || npc.type == Config.npcDefs.byName["Armored Hell Corruptor"].type || npc.type == 139)
								{
									if (num78 > 100f || npc.type == 42 || npc.type == Config.npcDefs.byName["Armored Hell Corruptor"].type)
									{
										npc.ai[0] += 1f;
										if (npc.ai[0] > 0f)
										{
									...
									if (num78 < 150f && (npc.type == 6 || npc.type == Config.npcDefs.byName["Armored Hell Corruptor"].type))
									{
								...
								else
								{
									...
									{
										if (npc.type == 6 || npc.type == Config.npcDefs.byName["Armored Hell Corruptor"].type)
										{
								...
								if (npc.type == 6 || npc.type == 23 || npc.type == 42 || npc.type == Config.npcDefs.byName["Armored Hell Corruptor"].type || npc.type == 139)
								{
									float num83 = 0.7f;
									if (npc.type == 6)
									{
								...
								if ((npc.type == 6 || npc.type == Config.npcDefs.byName["Armored Hell Corruptor"].type) && npc.wet)
								{
								...
								if (Main.netMode != 1 && npc.type == Config.npcDefs.byName["Armored Hell Corruptor"].type && !Main.player[npc.target].dead)
								{
									if (npc.justHit)
									{
										npc.localAI[0] = 0f;
									...
									if (npc.localAI[0] == 180f)
									{
										if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
										{
											NPC.NewNPC((int)(npc.position.X + (float)(npc.width / 2) + npc.velocity.X), (int)(npc.position.Y + (float)(npc.height / 2) + npc.velocity.Y), Config.npcDefs.byName["Vile Spit 2"].type, 0);
										}
										npc.localAI[0] = 0f;
									}
								}
								if ((Main.dayTime && npc.type != 6 && npc.type != 23 && npc.type != 42 && npc.type != 94 && npc.type != Config.npcDefs.byName["Armored Hell Corruptor"].type) || Main.player[npc.target].dead)
								{
...
public void NPCLoot()
{
	Gore.NewGore(npc.position,npc.velocity,"AHC Gore 1",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"AHC Gore 2",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"AHC Gore 3",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"AHC Gore 4",0.9f,-1);
}