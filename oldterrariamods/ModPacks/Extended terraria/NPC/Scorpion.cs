public bool SpawnNPC(int x, int y, int playerID)
{
	...
	//{
		if (Main.tileCount[53] >= 40 && !Main.player[playerID].zoneMeteor && !Main.player[playerID].zoneDungeon && y < (Main.rockLayer + 30) && Main.rand.Next(3) == 0)
		{
			return true;
		}
		else return false;
...
public void NPCLoot()
{
	Gore.NewGore(npc.position,npc.velocity,"Scorpion Body Gore",1f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Scorpion Leg Gore",1f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Scorpion Leg Gore",1f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Scorpion Leg Gore",1f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Scorpion Leg Gore",1f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Scorpion Tail Gore",1f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Scorpion Pincer Gore",1f,-1);
}
...
public void AI()
{
	...
	bool flag2 = false;
	bool flag3 = true;
	if (npc.type == 47 || npc.type == 67 || npc.type == 109 || npc.type == Config.npcDefs.byName["Scorpion"].type || npc.type == 111 || npc.type == 120)
	{
		flag3 = false;
	}
	if ((npc.type != Config.npcDefs.byName["Scorpion"].type && npc.type != 111) || npc.ai[2] <= 0f)
	{
	...
	if ((!Main.dayTime || (double)npc.position.Y > Main.worldSurface * 16.0 || npc.type == 26 || npc.type == 27 || npc.type == 28 || npc.type == 31 || npc.type == 47 || npc.type == 67 || npc.type == 73 || npc.type == 77 || npc.type == 78 || npc.type == 79 || npc.type == 80 || npc.type == Config.npcDefs.byName["Scorpion"].type || npc.type == 111 || npc.type == 120) && npc.ai[3] < (float)num5)
	{
		if ((npc.type == 3 || npc.type == 21 || npc.type == 31 || npc.type == 77 || npc.type == Config.npcDefs.byName["Scorpion"].type || npc.type == 132) && Main.rand.Next(1000) == 0)
		{
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						{
							if (npc.type != Config.npcDefs.byName["Scorpion"].type && npc.type != 111)
							{
	...
	if (npc.type == Config.npcDefs.byName["Scorpion"].type || npc.type == 111)
	{
		...
		else
		{
			...
			if (npc.ai[2] <= 0f && npc.velocity.Y == 0f && npc.ai[1] <= 0f && !Main.player[npc.target].dead && Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height) && Vector2.Distance(npc.position, Main.player[npc.target].position) <= 96f)
			{
				float num21 = 10f;
	...
	if (Main.netMode != 1 && npc.type == 120 && npc.ai[3] >= (float)num5)
	{
