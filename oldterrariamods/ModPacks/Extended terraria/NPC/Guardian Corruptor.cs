public bool SpawnNPC(int x, int y, int playerID) 
{
	...
	{
		if ((Main.invasionType != 1 && Main.invasionType != 2) && Main.player[playerID].townNPCs < 1 && Main.player[playerID].zoneEvil && !Main.player[playerID].zoneMeteor && y < (Main.maxTilesY - 200) && Main.rand.Next(10) == 0)
		{
			return true;
		}
		else return false;
	...
	if (Main.netMode != 1 && npc.type == Config.npcDefs.byName["Guardian Corruptor"].type && !Main.player[npc.target].dead)
	{
		...
		if (npc.localAI[0] == 180f)
		{
			if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
			{
				...
				int a = Projectile.NewProjectile(npc.position.X + (npc.width / 2), npc.position.Y + (npc.height * 0.5f), -(float)Math.Cos(rotation)*7f, -(float)Math.Sin(rotation)*7f, "Vile Spit 2", 70, 1f, npc.target);
				if (Main.netMode == 2)
				{
					NetMessage.SendData(27, -1, -1, "", a, 0f, 0f, 0f, 0);
				}
				...
				int b = Projectile.NewProjectile(npc.position.X + (npc.width / 2), npc.position.Y + (npc.height * 0.5f), -(float)Math.Cos(rotation2), -(float)Math.Sin(rotation2), "Vile Spit 2", 70, 1f, npc.target);
				...
				if (Main.netMode == 2)
				{
					NetMessage.SendData(27, -1, -1, "", b, 0f, 0f, 0f, 0);
				}
