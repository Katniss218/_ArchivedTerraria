public void AI()
{
												if (npc.target == 255)
												{
													...
													if (npc.type == Config.npcDefs.byName["Hallow Spit"].type)
													{
														num157 = 7f;
													}
												...
												if (npc.type == Config.npcDefs.byName["Hallow Spit"].type)
												{
													...
													if (npc.ai[0] == 2f)
													{
														...
														{
															int num162 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y + 2f), npc.width, npc.height, 58, 0f, 0f, 100, default(Color), 1.8f);
												...
												if (npc.type == Config.npcDefs.byName["Hallow Spit"].type && Collision.SolidCollision(npc.position, npc.width, npc.height))
												{
													if (Main.netMode != 1)
													{
												...
												for (int num168 = 0; num168 < 2; num168++)
												{
													...
													else
													{
														...
														else
														{
															if (npc.type == Config.npcDefs.byName["Hallow Spit"].type)
															{
																int num171 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y + 2f), npc.width, npc.height, 58, npc.velocity.X * 0.1f, npc.velocity.Y * 0.1f, 80, default(Color), 1.3f);
