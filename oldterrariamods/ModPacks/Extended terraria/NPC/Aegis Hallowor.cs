public bool SpawnNPC(int x, int y, int playerID) 
{
	...
	{
		if ((Main.invasionType != 1 && Main.invasionType != 2) && Main.player[playerID].townNPCs < 1 && Main.player[playerID].zoneHoly && !Main.player[playerID].zoneMeteor && y < (Main.maxTilesY - 200) && Main.rand.Next(10) == 0)
		{
			return true;
		}
		else return false;
...
public void AI()
{
	...
	{
		if (npc.type == Config.npcDefs.byName["Aegis Hallowor"].type)
		{
			num73 = 4.2f;
			num74 = 0.022f;
		}
	...
	if (npc.type == 6 || npc.type == 42 || npc.type == Config.npcDefs.byName["Aegis Hallowor"].type || npc.type == 139)
	{
		if (num78 > 100f || npc.type == 42 || npc.type == Config.npcDefs.byName["Aegis Hallowor"].type)
		{
			npc.ai[0] += 1f;
			if (npc.ai[0] > 0f)
			{
		...
		if (num78 < 150f && (npc.type == 6 || npc.type == Config.npcDefs.byName["Aegis Hallowor"].type))
		{
	...
	else
	{
		...
		{
			if (npc.type == 6 || npc.type == Config.npcDefs.byName["Aegis Hallowor"].type)
			{
	...
	if (npc.type == 6 || npc.type == 23 || npc.type == 42 || npc.type == Config.npcDefs.byName["Aegis Hallowor"].type || npc.type == 139)
	{
		float num83 = 0.7f;
		if (npc.type == 6)
		{
	...
	if ((npc.type == 6 || npc.type == Config.npcDefs.byName["Aegis Hallowor"].type) && npc.wet)
	{
	...
	if (Main.netMode != 1 && npc.type == Config.npcDefs.byName["Aegis Hallowor"].type && !Main.player[npc.target].dead)
	{
		if (npc.justHit)
		{
			npc.localAI[0] = 0f;
		...
		if (npc.localAI[0] == 180f)
		{
			if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
			{
				...
				int a = Projectile.NewProjectile(npc.position.X + (npc.width / 2), npc.position.Y + (npc.height * 0.5f), -(float)Math.Cos(rotation)*7f, -(float)Math.Sin(rotation)*7f, "Hallow Spit 2", 70, 1f, npc.target);
				if (Main.netMode == 2)
				{
				...
				int b = Projectile.NewProjectile(npc.position.X + (npc.width / 2), npc.position.Y + (npc.height * 0.5f), -(float)Math.Cos(rotation2), -(float)Math.Sin(rotation2), "Hallow Spit 2", 70, 1f, npc.target);
				...
				NPC.NewNPC((int)(npc.position.X + (float)(npc.width / 2) + npc.velocity.X), (int)(npc.position.Y + (float)(npc.height / 2) + npc.velocity.Y), Config.npcDefs.byName["Hallow Spit"].type, 0);
			}
			npc.localAI[0] = 0f;
		}
	}
	if ((Main.dayTime && npc.type != 6 && npc.type != 23 && npc.type != 42 && npc.type != 94 && npc.type != Config.npcDefs.byName["Aegis Hallowor"].type) || Main.player[npc.target].dead)
	{
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= C; i++)
	{
		int D = Dust.NewDust(npc.position, R.Width, R.Height, 57, 0, 0, 100, new Color(), 2f);
	...
	{
		int D2 = Dust.NewDust(npc.position, R.Width, R.Height, 15, 0, 0, 100, new Color(), 2f);
