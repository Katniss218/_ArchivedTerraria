public bool SpawnNPC(int x, int y, int playerID) 
{
	if (Main.hardMode)
	{
		if ((Main.invasionType != 1 && Main.invasionType != 2) && Main.player[playerID].townNPCs < 1 && Main.player[playerID].zoneHoly && !Main.player[playerID].zoneMeteor && !Main.player[playerID].zoneDungeon && y < (Main.maxTilesY - 200) && Main.rand.Next(7) == 0)
		{
			return true;
		}
		else return false;
...
public void AI()
{
								...
								{
									if (npc.type == Config.npcDefs.byName["Hallowor"].type)
									{
										num73 = 4.2f;
										num74 = 0.022f;
									}
								...
								if (npc.type == 6 || npc.type == 42 || npc.type == Config.npcDefs.byName["Hallowor"].type || npc.type == 139)
								{
									if (num78 > 100f || npc.type == 42 || npc.type == Config.npcDefs.byName["Hallowor"].type)
									{
										npc.ai[0] += 1f;
										if (npc.ai[0] > 0f)
										{
									...
									if (num78 < 150f && (npc.type == 6 || npc.type == Config.npcDefs.byName["Hallowor"].type))
									{
								...
								else
								{
									...
									{
										if (npc.type == 6 || npc.type == Config.npcDefs.byName["Hallowor"].type)
										{
								...
								if (npc.type == 6 || npc.type == 23 || npc.type == 42 || npc.type == Config.npcDefs.byName["Hallowor"].type || npc.type == 139)
								{
									float num83 = 0.7f;
									if (npc.type == 6)
									{
								...
								if ((npc.type == 6 || npc.type == Config.npcDefs.byName["Hallowor"].type) && npc.wet)
								{
								...
								if (Main.netMode != 1 && npc.type == Config.npcDefs.byName["Hallowor"].type && !Main.player[npc.target].dead)
								{
									if (npc.justHit)
									{
										npc.localAI[0] = 0f;
								...
								if ((Main.dayTime && npc.type != 6 && npc.type != 23 && npc.type != 42 && npc.type != 94 && npc.type != Config.npcDefs.byName["Hallowor"].type) || Main.player[npc.target].dead)
								{
...
public void NPCLoot()
{
	...
	//Gore.NewGore(npc.position,npc.velocity,"Hallowor Gore 1",1f,-1);
	//Gore.NewGore(npc.position,npc.velocity,"Hallowor Gore 2",1f,-1);
	//Gore.NewGore(npc.position,npc.velocity,"Hallowor Gore 3",1f,-1);
	//Gore.NewGore(npc.position,npc.velocity,"Hallowor Gore 4",1f,-1);
}