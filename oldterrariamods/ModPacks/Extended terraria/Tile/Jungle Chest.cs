...
public void MouseOverTile(int x,int y, Player player) {
	...
	player.showItemIcon2 = Config.itemDefs.byName["Jungle Chest"].type;
}
...
public bool CanDestroyTile(int x, int y) {
	while(Main.tile[x,y].frameX>0) x--;
	while(Main.tile[x,y].frameY>0) y--;
	int ID3= Chest.FindChest(x, y);
	if(ID3==-1) return true;
	for(int i=0;i<Chest.maxItems;i++) {
		if(Main.chest[ID3].item[i].type > 0) return false;
	}
...
public void MouseOverTile(int x,int y,Player P)
{
	P.noThrow = 2;
	P.showItemIcon = true;
	P.showItemIcon2 = Config.itemDefs.byName["Jungle Chest"].type;
}*/