...
public void AI()
{
	...
	if (projectile.ai[0] == 0f)
	{
		...
		if (projectile.alpha <= 0)
		{
			...
			{
				projectile.ai[1] += 1f;
				projectile.position += projectile.velocity * 0f;
			}
	...
	else
	{
		...
		{
			projectile.Kill();
            Projectile.NewProjectile(projectile.position.X + (float)(projectile.width / 2), projectile.position.Y + (float)(projectile.height / 2), 0, 0, Config.projectileID["Spell Lightning 3 Bolt 3"], (int) (this.projectile.damage), 8f, projectile.owner);
			return;
