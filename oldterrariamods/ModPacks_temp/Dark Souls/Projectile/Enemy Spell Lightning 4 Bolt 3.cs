...
public void AI()
{
	...
	else
	{
		...
		{
			projectile.Kill();
            Projectile.NewProjectile(projectile.position.X + (float)(projectile.width / 2), projectile.position.Y + (float)(projectile.height / 2), 0, 0, Config.projectileID["Enemy Spell Lightning 4 Bolt 4"], (int) ((Main.player[projectile.owner].statMana/10) + this.projectile.damage), 8f, projectile.owner);
			return;
		}
	}
}
