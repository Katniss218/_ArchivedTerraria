    ...
    if (Main.myPlayer == projectile.owner && projectile.ai[0] == 0f)
    {
        ...
        else
        {
            if (projectile.ai[0] == 0f)
            {
			    ...
			    {
			        projectile.Kill();
                    Projectile.NewProjectile(projectile.position.X + (float)(projectile.width), projectile.position.Y + (float)(projectile.height), 0, 0, Config.projectileID["Spell Fire Field"], 30, 3f, projectile.owner);
                }
		    }
	    }
    }
