...
public void AI()
{
	...
	{                     
		projectile.Kill();                        
        if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)((projectile.width / 2)+30), projectile.position.Y + (float)(projectile.height / 2), 0, 0, "Enemy Ultima Explosion", 200, 10f, projectile.owner);
		return;
	}
