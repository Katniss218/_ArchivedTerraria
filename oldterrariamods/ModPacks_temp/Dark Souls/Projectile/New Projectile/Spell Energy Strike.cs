...
public void Kill()
{
	...
	projectile.timeLeft = 0;
    {
        ...
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*4), projectile.position.Y + (float)(projectile.height), 0, 0, "Spell Great Energy Strike", (int) (10 * Main.player[projectile.owner].magicDamage), 3f, projectile.owner);
        }
        else
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*-4), projectile.position.Y + (float)(projectile.height), 0, 0, "Spell Great Energy Strike", (int) (10 * Main.player[projectile.owner].magicDamage), 3f, projectile.owner);
