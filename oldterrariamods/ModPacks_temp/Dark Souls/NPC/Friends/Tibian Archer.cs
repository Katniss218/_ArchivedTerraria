...
public static bool TownSpawn() 
{
    for (int pInv = 0; pInv < 48; pInv++)
    {
        ...
        {
            if (!NPC.AnyNPCs(Config.npcDefs.byName["Tibian Archer"].type) && !NPC.AnyNPCs(Config.npcDefs.byName["Tibian Archer"].type)) return true;
        }
    }
    return false;
}
...
public static string Chat() 
{
    ...
    if(x==1)
    {
        return "Tibia, a green island. There it is wonderful to walk into the forests and to hunt with a bow.";
    }
    if(x==2)
    {
    ...
    if(x==5)
    {
        return "Please show respect to Eloise. I don't want to have to hurt you.";
    }
    if(x==6)
    {
        return "Amazons and dworcs are a real threat.";
    }
...
public static void SetupShop(Chest chest) 
{
 ...
 if (NPC.downedBoss1)
    ...
    {
            chest.item[index].SetDefaults("Forgotten Ice Bow Scroll");
	    index++;
            chest.item[index].SetDefaults("Forgotten Thunder Bow Scroll");
	    index++;
    }
