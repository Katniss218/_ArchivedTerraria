...
public bool SpawnNPC(int x,int y,int PID)
{
	...
	{
	    if (Main.npc[num36].active && Main.npc[num36].type == Config.npcDefs.byName["Oolacile Knight"].type)
	    {
	        return false;
		}
	}

      if(Jungle && Main.bloodMoon && ModWorld.superHardmode && AboveEarth && !ModWorld.Slayed.ContainsKey("Oolacile Knight") &&  Main.rand.Next(4000)==1) 
	...
	if(Jungle && Main.bloodMoon && ModWorld.superHardmode && !AboveEarth && !ModWorld.Slayed.ContainsKey("Oolacile Knight") && Main.rand.Next(2000)==1) 
	...
	if(Meteor && Main.bloodMoon && ModWorld.superHardmode && !Main.dayTime && InGrayLayer && !ModWorld.Slayed.ContainsKey("Oolacile Knight") && Main.rand.Next(100)==1) 
...
if(Main.netMode != 1)
{
	...
	//customAi5 += (Main.rand.Next(2, 5) * 0.1f) * npc.scale;
		...
		if ((customspawn1 < 1) && Main.rand.Next(650)==1)
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Oolacile Demon", 0);
	...
	if (breakCombo == true || (enraged == true && Main.rand.Next(500)==1)) 
        {
...
#region Projectiles
    ...
    if (breath) 
	{
				...
				int num54 = Projectile.NewProjectile(npc.position.X + (npc.width * 0.5f), npc.position.Y + (npc.height / 2),(float)((Math.Cos(rotation) * 25)*-1),(float)((Math.Sin(rotation) * 25)*-1), "Cursed Dragon's Breath", 55, 0f, Main.myPlayer);
...
if (Main.rand.Next(35) == 0) 
	{
		int num65 = Projectile.NewProjectile(npc.Center.X+Main.rand.Next(-500,500), npc.Center.Y+Main.rand.Next(-500,500), 0, 0, "Dark Explosion 2", 70, 0f, Main.myPlayer);
	}
		...
		if (Main.rand.Next(300)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        int type = Config.projectileID["Earth Trident"];//44;//0x37; //14;
...
public void DamagePlayer(Player player, ref int damage) //hook works!
{
	...
	{
		player.AddBuff("Curse Buildup", 36000, false); //-20 HP curse
	}
...
public void PostDraw(SpriteBatch spriteBatch)
{
			...
			if(npc.spriteDirection == 1){
				...
				for(int i=15;i>-1;i--)
				{
					...
					spriteBatch.Draw(Main.goreTexture[Config.goreID["Oolacile Knight Glow"]],
...
public void NPCLoot()
{
	Gore.NewGore(npc.position,npc.velocity,"Oolacile Knight Gore 1",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Oolacile Knight Gore 2",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Oolacile Knight Gore 3",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Oolacile Knight Gore 4",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Oolacile Knight Gore 2",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Oolacile Knight Gore 3",0.9f,-1);
