...
public bool SpawnNPC(int x,int y,int PID)
{
    ...
    if(ModWorld.superHardmode && !Meteor && !Jungle && !Dungeon && !Corruption && FrozenOcean && Main.rand.Next(30)==1) return true;

	if(ModWorld.superHardmode && Hallow && !Jungle && !Dungeon && !Corruption && FrozenOcean && Main.rand.Next(20)==1) return true;

	if(ModWorld.superHardmode && Hallow && !Jungle && Dungeon && !Corruption && Main.rand.Next(30)==1) return true;

	if(ModWorld.superHardmode && Hallow && (InBrownLayer || InGrayLayer) && Main.rand.Next(30)==1) return true;
	
	if(ModWorld.superHardmode && Main.bloodMoon && !Meteor && !Jungle && !InHell && !Corruption && FrozenOcean && Main.rand.Next(8)==1) return true;

	if(ModWorld.superHardmode && Main.bloodMoon && !Meteor && !InHell && !Corruption && Hallow && Main.rand.Next(12)==1) return true;
...
// set parameters
    ...
    int projectile_id = Config.projectileID["Enemy Crystal Knight Bolt"]; // projectile id: 82(Flaming Arrow) for Skeleton Archer, 81(Wooden Arrow) for Goblin Archer, 75(Happy Bomb) for Clown
...
Gore.NewGore(npc.position,npc.velocity,"Crystal Knight Gore 1",1.1f,-1);
Gore.NewGore(npc.position,npc.velocity,"Crystal Knight Gore 2",1.1f,-1);
Gore.NewGore(npc.position,npc.velocity,"Crystal Knight Gore 2",1.1f,-1);
Gore.NewGore(npc.position,npc.velocity,"Crystal Knight Gore 2",1.1f,-1);
