...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      if(ModWorld.superHardmode && !Meteor && !Jungle && !Dungeon && !Corruption && !AboveEarth && !InHell && !Hallow && !Ocean && Main.rand.Next(30)==1) return true;

	  if(ModWorld.superHardmode && !Meteor && !Jungle && !Dungeon && !Corruption && !AboveEarth && !InHell && !Hallow && !Ocean && Main.bloodMoon && Main.rand.Next(15)==1) return true;

	  if(ModWorld.superHardmode && !Meteor && !Jungle && !Dungeon && !Corruption && !AboveEarth && !InHell && !Hallow && !Ocean && !Main.dayTime && Main.rand.Next(20)==1) return true;
...
// set parameters
    ...
    int projectile_id = Config.projectileID["Enemy Spell Abyss Storm Wave"]; // projectile id: 82(Flaming Arrow) for Skeleton Archer, 81(Wooden Arrow) for Goblin Archer, 75(Happy Bomb) for Clown
