...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    bool BeforeThreeAfterSeven = (x < Main.maxTilesX*0.3f) || (x > Main.maxTilesX*0.7f); //Before 3/10ths or after 7/10ths width

    if (ModWorld.superHardmode && Main.bloodMoon && oUnderworld && Main.rand.Next(10)==1) return true;
    if (ModWorld.superHardmode && oUnderworld && Main.rand.Next(80)==1) return true;
    if (ModWorld.superHardmode && oMagmaCavern && Main.rand.Next(400)==1) return true;
	if (ModWorld.superHardmode && BeforeThreeAfterSeven && oUnderworld && Main.rand.Next(30) == 0) return true;
    if (ModWorld.superHardmode && BeforeThreeAfterSeven && oUnderworld && Main.bloodMoon && Main.rand.Next(5) == 0) return true;
...
#region Projectiles
        ...
        if (customAi1 >= 10f)
        {
	        ...
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Crazed Demon Spirit", 0);
		        ...
		        if (Main.netMode == 2)
		        {
		            NetMessage.SendData(23, -1, -1, "Crazed Demon Spirit", Spawned, 0f, 0f, 0f, 0);
		        }
	        }
