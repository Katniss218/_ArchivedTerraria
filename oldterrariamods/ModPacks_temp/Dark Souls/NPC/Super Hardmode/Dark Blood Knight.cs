...
public bool SpawnNPC(int x,int y,int PID)
{
    ...
    if(ModWorld.superHardmode && Jungle && !Dungeon && !Corruption && !AboveEarth && !Ocean && Main.rand.Next(50)==1) return true;

	if(ModWorld.superHardmode && Main.bloodMoon && !Meteor && Jungle && !Dungeon && !Corruption && !AboveEarth && !Ocean && Main.rand.Next(20)==1) return true;

	if(ModWorld.superHardmode && !Main.dayTime && Jungle && !Dungeon && !Corruption && !AboveEarth && !Ocean && Main.rand.Next(30)==1) return true;

	if(ModWorld.superHardmode && !Main.dayTime && Jungle && !Dungeon && !Corruption && AboveEarth && !Ocean && Main.rand.Next(80)==1) return true;
...
// set parameters
    ...
    int projectile_damage = 90;  //  projectile dmg: 35 for Skeleton Archer, 11 for Goblin Archer
    int projectile_id = Config.projectileID["Enemy Black Fire Wave"]; // projectile id: 82(Flaming Arrow) for Skeleton Archer, 81(Wooden Arrow) for Goblin Archer, 75(Happy Bomb) for Clown
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Man Hunter Gore 1", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Man Hunter Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Man Hunter Gore 3", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Man Hunter Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Man Hunter Gore 3", 1.1f, -1);
    }
}
