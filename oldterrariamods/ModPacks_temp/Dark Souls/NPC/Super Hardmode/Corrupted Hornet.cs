public bool SpawnNPC(int x, int y, int playerID)
{
	...
	{
		if (y > Main.rockLayer && y < Main.maxTilesY - 200 && Main.evilTiles > 20 && Main.jungleTiles > 50 && !Main.player[playerID].zoneDungeon && Main.rand.Next(5) == 0)
		{
			return true;
		}
		else return false;
	...
	if (npc.ai[3] >= 200) //200 was 240
	{
		if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
		{
			...
			if ((num88 < 0f && npc.velocity.X < 0f) || (num88 > 0f && npc.velocity.X > 0f))
			{
				...
				num88 *= num90;
				num89 *= num90;
				int num91 = 120; //(int)(13f * npc.scale);
				int num92 = 96; //55 is id of stinger 96 is spantasm cursed flame
...
public void NPCLoot()
{
	Gore.NewGore(npc.position,npc.velocity,"Corrupt Hornet Gore 1",1.1f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Corrupt Hornet Gore 2",1.1f,-1);
