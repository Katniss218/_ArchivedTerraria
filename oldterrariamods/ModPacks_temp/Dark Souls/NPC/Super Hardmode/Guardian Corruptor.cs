...
public bool SpawnNPC(int x,int y,int PID)
{
	...
	if(ModWorld.superHardmode && Corruption && Main.rand.Next(30)==1) return true;
	
	if(ModWorld.superHardmode && AboveEarth && Corruption && !Main.dayTime && Main.rand.Next(20)==1) return true;

	if(ModWorld.superHardmode && InGrayLayer && Corruption && !Main.dayTime && Main.rand.Next(15)==1) return true;
...
public void AI()
{
	...
	{
		if (npc.type == Config.npcDefs.byName["Guardian Corruptor"].type)
		{
			num73 = 4.2f;
			num74 = 0.022f;
		}
	...
	if (npc.type == 6 || npc.type == 42 || npc.type == Config.npcDefs.byName["Guardian Corruptor"].type || npc.type == 139)
	{
		if (num78 > 100f || npc.type == 42 || npc.type == Config.npcDefs.byName["Guardian Corruptor"].type)
		{
			npc.ai[0] += 1f;
			if (npc.ai[0] > 0f)
			{
		...
		if (num78 < 150f && (npc.type == 6 || npc.type == Config.npcDefs.byName["Guardian Corruptor"].type))
		{
	...
	else
	{
		...
		{
			if (npc.type == 6 || npc.type == Config.npcDefs.byName["Guardian Corruptor"].type)
			{
	...
	if (npc.type == 6 || npc.type == 23 || npc.type == 42 || npc.type == Config.npcDefs.byName["Guardian Corruptor"].type || npc.type == 139)
	{
		float num83 = 0.7f;
		if (npc.type == 6)
		{
		...
		if (npc.type == 23)
		{
			int num84 = Dust.NewDust(new Vector2(npc.position.X - npc.velocity.X, npc.position.Y - npc.velocity.Y), npc.width, npc.height, 6, npc.velocity.X * 0.2f, npc.velocity.Y * 0.2f, 100, default(Color), 2f);
		...
		else
		{
			...
			{
				int num85 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y + (float)npc.height * 0.25f), npc.width, (int)((float)npc.height * 0.5f), 18, npc.velocity.X, 2f, 75, npc.color, npc.scale);
	...
	else
	{
		...
		{
			int num86 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y + (float)npc.height * 0.25f), npc.width, (int)((float)npc.height * 0.5f), 5, npc.velocity.X, 2f, 0, default(Color), 1f);
	...
	if ((npc.type == 6 || npc.type == Config.npcDefs.byName["Guardian Corruptor"].type) && npc.wet)
	{
	...
	if (Main.netMode != 1 && npc.type == Config.npcDefs.byName["Guardian Corruptor"].type && !Main.player[npc.target].dead)
	{
		//if (npc.justHit)
		//{
		//	npc.localAI[0] = 0f;
		//}
		npc.localAI[0] += 1f;
		if (npc.localAI[0] == 50f) //was 180f
		{
			if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
			{
				Player T = Main.player[npc.target];
				Vector2 vector8 = new Vector2(npc.Center.X, npc.Center.Y);
				float rotation = (float) Math.Atan2(vector8.Y-(T.position.Y+(T.height * 0.5f)+40f), vector8.X-(T.position.X+(T.width * 0.5f)+40f));
				int a = Projectile.NewProjectile(npc.Center.X, npc.Center.Y, -(float)Math.Cos(rotation)*7f, -(float)Math.Sin(rotation)*7f, "Vile Spit 2", 100, 1f, npc.target);
				//Main.projectile[a].velocity.X *= 2f;
				//Main.projectile[a].velocity.Y *= 2f;
				float rotation2 = (float) Math.Atan2(vector8.Y-(T.position.Y+(T.height * 0.5f)-40f), vector8.X-(T.position.X+(T.width * 0.5f)-40f));
				int b = Projectile.NewProjectile(npc.position.X + (npc.width / 2), npc.position.Y + (npc.height * 0.5f), -(float)Math.Cos(rotation2), -(float)Math.Sin(rotation2), "Vile Spit 2", 100, 1f, npc.target); //90 was 70
				Main.projectile[b].velocity.X *= 7f; //(float)Math.Cos(npc.velocity.X - 1.57f / 2f) * 3f;
				Main.projectile[b].velocity.Y *= 7f; //(float)Math.Sin(npc.velocity.X - 1.57f / 2f) * 3f;
	...
	if ((Main.dayTime && npc.type != 6 && npc.type != 23 && npc.type != 42 && npc.type != 94 && npc.type != Config.npcDefs.byName["Guardian Corruptor"].type) || Main.player[npc.target].dead)
	{
...
public void NPCLoot()
{
	Gore.NewGore(npc.position,npc.velocity,"Guardian Corruptor Gore 1",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Guardian Corruptor Gore 2",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Guardian Corruptor Gore 3",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Guardian Corruptor Gore 4",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Blood Splat",0.9f,-1);	
	Gore.NewGore(npc.position,npc.velocity,"Blood Splat",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Blood Splat",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Blood Splat",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Blood Splat",0.9f,-1);
