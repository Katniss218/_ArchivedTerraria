...
public bool SpawnNPC(int x,int y,int PID)
{
	...
	if(ModWorld.superHardmode && Dungeon && Main.rand.Next(100)==1) 
	...
	if(ModWorld.superHardmode && InHell && Main.rand.Next(100)==1) 
...
if (Main.rand.Next(275) == 0) 
    ...
    if (breath) 
	{
        ...
        //for (int pcy = 0; pcy < 10; pcy++) {
            Projectile.NewProjectile(npc.position.X + (float)npc.width / 2f, npc.position.Y + (float)npc.height / 2f, npc.velocity.X * 1f + (float)Main.rand.Next(-2, 3), npc.velocity.Y * 3f + (float)Main.rand.Next(-2, 3),  Config.projDefs.byName["Cursed Dragon's Breath"].type, 75, 1.2f, 255); //96 was Config.projDefs.byName["Enemy Light Spirit"].type,  85 is damage
        //}
        breathCD--;
        //}
    }
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Red Knight Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Red Knight Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Red Knight Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Red Knight Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Red Knight Gore 3", 1f, -1);
