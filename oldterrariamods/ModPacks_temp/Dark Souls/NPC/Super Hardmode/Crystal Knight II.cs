...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      if(ModWorld.superHardmode && !Meteor && !Jungle && !Dungeon && !Corruption && Ocean && Main.rand.Next(100)==1) return true;


	if(ModWorld.superHardmode && Main.bloodMoon && !Meteor && !Jungle && !Dungeon && !Corruption && !AboveEarth && Ocean && Main.rand.Next(30)==1) return true;
...
// set parameters
    ...
    int projectile_damage = 85;  //  projectile dmg: 35 for Skeleton Archer, 11 for Goblin Archer
    int projectile_id = Config.projectileID["Enemy Crystal Knight Bolt II"]; // projectile id: 82(Flaming Arrow) for Skeleton Archer, 81(Wooden Arrow) for Goblin Archer, 75(Happy Bomb) for Clown
