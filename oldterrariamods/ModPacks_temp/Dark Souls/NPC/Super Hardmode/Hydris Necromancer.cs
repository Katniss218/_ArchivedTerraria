...
public static bool SpawnNPC(int x, int y, int PID) 
{
	...
	if (ModWorld.superHardmode && (oUnderground || oCavern || oMagmaCavern))
    {
        if (Hallow && Main.rand.Next(90)==1) return true;
		if (Hallow && Main.bloodMoon && Main.rand.Next(10)==1) return true;
        if ((x < Main.maxTilesX*0.35f || x > Main.maxTilesX*0.75f) && Main.rand.Next(50)==1) return true;
	    return false;
    }
	else if (ModWorld.superHardmode && oUnderworld)
    {
        if (Main.rand.Next(200)==1) return true;
        if ((x < Main.maxTilesX*0.35f || x > Main.maxTilesX*0.75f) && Main.rand.Next(30)==1) return true;
	    return false;
    }
    return false;
}
...
public void DamagePlayer(Player player, ref int damage) //hook works!
{
	...
	{
		player.AddBuff("Frozen", 120, false);
...
#region Projectiles
        ...
        if (customAi1 >= 10f)
        {
	        ...
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Hydris Elemental", 0);
