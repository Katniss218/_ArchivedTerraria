...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      // these are all the regular stuff you get , now lets see......
        ...
        if(ModWorld.superHardmode && !Main.dayTime && Jungle && AboveEarth && Main.rand.Next(30)==1) return true;

	if(ModWorld.superHardmode && !Main.dayTime && Jungle && InBrownLayer && Main.rand.Next(20)==1) return true;

	if(ModWorld.superHardmode && !Main.dayTime && Jungle && InGrayLayer && Main.rand.Next(15)==1) return true;

        if(ModWorld.superHardmode && !Main.dayTime && Jungle && Main.bloodMoon && Main.rand.Next(5)==1) return true;
...
if(Main.netMode != 1)
{
	...
	//customAi5 += (Main.rand.Next(2, 5) * 0.1f) * npc.scale;
		...
		if ((customspawn1 < 4) && Main.rand.Next(550)==1)
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Vampire Bat", 0);
