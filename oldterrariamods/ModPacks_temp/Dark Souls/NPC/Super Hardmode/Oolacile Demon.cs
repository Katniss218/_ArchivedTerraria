...
public bool SpawnNPC(int x,int y,int PID)
{
	...
	{
	    if (Main.npc[num36].active && Main.npc[num36].type == Config.npcDefs.byName["Oolacile Demon"].type)
	    {
	        return false;
		}
	}
 
	if(ModWorld.superHardmode && !Main.dayTime && InHell && Main.rand.Next(13)==1) return true;

	if(ModWorld.superHardmode && Main.bloodMoon && InHell && Main.rand.Next(8)==1) return true;

	if(ModWorld.superHardmode && Main.dayTime && InHell && Main.rand.Next(20)==1) return true;
...
//	#endregion
    ...
    if (breath) 
	{
				...
				int num54 = Projectile.NewProjectile(npc.Center.X, npc.Center.Y-5,(float)((Math.Cos(rotation) * 25)*-1),(float)((Math.Sin(rotation) * 25)*-1), "Enemy Cursed Breath", 75, 0f, Main.myPlayer);
...
public void DamagePlayer(Player player, ref int damage) //hook works!
{
	...
	if (Main.rand.Next(4) == 0)
	{
		...
		player.AddBuff("Fracturing Armor", 3600, false); //armor reduced on hit
        player.AddBuff("Chilled", 300, false); //chilled
...
public void NPCLoot()
{
	Gore.NewGore(npc.position,npc.velocity,"Gaibon Gore 1",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Gaibon Gore 2",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Gaibon Gore 3",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Gaibon Gore 4",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Gaibon Gore 2",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Gaibon Gore 3",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Gaibon Gore 4",0.9f,-1);
