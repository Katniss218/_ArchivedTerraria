...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      if((Meteor || Jungle) && ModWorld.superHardmode && !Dungeon && !Corruption && AboveEarth && Main.rand.Next(800)==1) return true;

	if((Meteor || Jungle) && ModWorld.superHardmode && !Dungeon && !Corruption && InBrownLayer && Main.rand.Next(400)==1) return true;

	if((Meteor || Jungle) && ModWorld.superHardmode && !Dungeon && !Corruption && InGrayLayer && Main.rand.Next(100)==1) return true;


	if(ModWorld.superHardmode && Jungle && !Corruption && !Ocean && Main.bloodMoon && Main.rand.Next(30)==1) return true;
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
			    ...
			    if (Main.rand.Next(200)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        int type = Config.projectileID["Enemy Spell Abyss Storm Ball"];//44;//0x37; //14;
				        int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
