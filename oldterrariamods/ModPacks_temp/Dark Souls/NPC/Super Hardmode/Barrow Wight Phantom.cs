...
if (Main.rand.Next(2075) == 0) 
    ...
    if (breath) 
	{
        ...
        //for (int pcy = 0; pcy < 10; pcy++) {
            Projectile.NewProjectile(npc.position.X + (float)npc.width / 2f, npc.position.Y + (float)npc.height / 2f, npc.velocity.X * 2f + (float)Main.rand.Next(-2, 3), npc.velocity.Y * 3f + (float)Main.rand.Next(-2, 3),  Config.projDefs.byName["Frozen Dragon's Breath"].type, 65, 1.2f, 255); //96 was Config.projDefs.byName["Enemy Light Spirit"].type,  85 is damage
        //}
        breathCD--;
        //}
    }
...
public void DamagePlayer(Player player, ref int damage) //hook works!
{
	if (Main.rand.Next(0) == 0)
	{
	   ...
	   player.AddBuff("Powerful Curse Buildup", 36000, false); //-100 life after several hits	
	}

	if (Main.rand.Next(4) == 0)
	{
                
		player.AddBuff(36, 300, false); //broken armor
