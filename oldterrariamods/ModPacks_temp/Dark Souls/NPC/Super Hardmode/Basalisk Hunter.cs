...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      if(ModWorld.superHardmode && Meteor && !Dungeon && !Corruption && !Jungle && (InBrownLayer || InGrayLayer) && Main.rand.Next(20)==1) return true;

      if(ModWorld.superHardmode && !Main.dayTime && Corruption && Main.rand.Next(20)==1) return true;

      if(ModWorld.superHardmode && !Jungle && (Corruption || Meteor) && Main.bloodMoon && (InBrownLayer || InGrayLayer || AboveEarth) && Main.rand.Next(5)==1) return true;

      if(ModWorld.superHardmode && Corruption && !Main.dayTime && !Dungeon && InGrayLayer && Main.rand.Next(10)==1) return true;
	
	  if(ModWorld.superHardmode && AboveEarth && !Main.dayTime && Main.rand.Next(25)==1) return true;
...
#region Projectiles
    ...
    if (breath) 
	{
				...
				int num54 = Projectile.NewProjectile(npc.Center.X, npc.Center.Y,(float)((Math.Cos(rotation) * 15)*-1),(float)((Math.Sin(rotation) * 15)*-1), "Enemy Cursed Breath", 50, 0f, Main.myPlayer);
		...
		//Projectile.NewProjectile(npc.position.X + (float)npc.width / 2f, npc.position.Y + (float)npc.height / 2f, speedX, speedY,  Config.projDefs.byName["Enemy Cursed Breath"].type, 75, 0f, Main.myPlayer); //96 was Config.projDefs.byName["Enemy Light Spirit"].type,  85 is damage
				...
				//	speedY *= num51;
				//	int damage = 70;
				//	int type = Config.projectileID["Enemy Cursed Breath"];//44;//0x37; //14;
