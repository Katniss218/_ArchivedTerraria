public void AI()
{
    ...
    {
    int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 57, npc.velocity.X, npc.velocity.Y, 200, Color.White, 1f);
    ...
    {
        if (Main.player[npc.target].buffType[num36] == Config.buffDefs.ID["Miakoda"])
        {
        buffOn = true;
