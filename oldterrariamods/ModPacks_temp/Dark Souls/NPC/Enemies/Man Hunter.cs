...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      if(!Meteor && Jungle && !Dungeon && !Corruption && AboveEarth && Main.rand.Next(15)==1) return true;

	if(!Meteor && Jungle && !Dungeon && !Corruption && InBrownLayer && Main.rand.Next(35)==1) return true;

	if(!Meteor && Jungle && !Corruption && InGrayLayer && Main.rand.Next(45)==1) return true;


	if(Main.hardMode && !Meteor && !Corruption && !Ocean && Main.rand.Next(50)==1) return true;
