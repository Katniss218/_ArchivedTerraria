...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      if(!Meteor && !Dungeon && !Corruption && AboveEarth && !Main.dayTime && Main.rand.Next(1000)==1) return true;

	if(Main.hardMode && Meteor && !Corruption && InBrownLayer && Main.rand.Next(150)==1) return true;

	if(Main.hardMode && !Main.dayTime && Dungeon && !Corruption && InGrayLayer && Main.rand.Next(300)==1) return true;

	if(Main.hardMode && !Corruption && !Ocean && !Main.dayTime && Main.rand.Next(500)==1) return true;

        if(Main.hardMode && InHell && !Main.dayTime && Main.rand.Next(100)==1) return true;

        if(ModWorld.superHardmode && Dungeon && Main.bloodMoon && Main.rand.Next(10)==1) return true;
