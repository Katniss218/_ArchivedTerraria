...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (oUnderworld)
    {
	    if (!Main.dayTime && !Main.hardMode && !ModWorld.superHardmode)
        {
            if (Main.rand.Next(21000)==1) return true;
	        else if ((x < Main.maxTilesX*0.35f || x > Main.maxTilesX*0.75f) && Main.rand.Next(10000)==1) return true;
	        return false;
        }
        
		if (Main.hardMode && !ModWorld.superHardmode)
        {
			if (ModWorld.Slayed.ContainsKey("The Hunter") && !ModWorld.Slayed.ContainsKey("Ancient Demon") && Main.rand.Next(100)==1) return true;
            if (ModWorld.Slayed.ContainsKey("The Hunter") && Main.rand.Next(2000)==1) return true;
			if (ModWorld.Slayed.ContainsKey("The Hunter") && !Main.dayTime && Main.rand.Next(1000)==1) return true;
	        else if ((x < Main.maxTilesX*0.25f || x > Main.maxTilesX*0.75f) && ModWorld.Slayed.ContainsKey("The Rage") && Main.rand.Next(500)==1) return true;
	        return false;
        }
        return false;
    }
...
#region Projectiles
        ...
        if (customAi1 >= 10f)
        {
	        ...
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Werewolf", 0);
		        ...
		        if (Main.netMode == 2)
		        {
		            NetMessage.SendData(23, -1, -1, "Werewolf", Spawned, 0f, 0f, 0f, 0);
		        }
	        }
		    if (Main.rand.Next(1550)==1)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
				    int damage = 50;
				    int type = Config.projectileID["Poison Crystal Fire"];//44;//0x37; //14;
 			...
 			if (Main.rand.Next(150)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        int type = Config.projectileID["Enemy Black Fire"];//44;//0x37; //14;
				        int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
