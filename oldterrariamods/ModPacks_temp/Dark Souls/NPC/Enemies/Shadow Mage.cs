...
public static bool SpawnNPC(int x, int y, int playerID)
{
    ...
    if (Main.hardMode && Main.player[playerID].zoneDungeon || Main.hardMode && underworld)
    {
	...
	if (npc.velocity.Y == 0f) // no jump/fall
	{
        ...
        if (Main.netMode != 2)
        {
                if (npc.ai[0] >= 12 && npc.ai[2] < 5)
                {
                        ...
                        int damage = 32;
                        int type = Config.projectileID["The Oracle"];
        ...
        if (npc.ai[3] >= 30) //how often the crystal attack can happen in frames per second
        {
                if (Main.rand.Next(2)==0) //1 in 2 chance boss will use attack when it flies down on top of you
                {
                   ...
                   if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
                   {
					...
					int Paraspawn = 0;
				if (Random == 0) Paraspawn = NPC.NewNPC((int) Main.player[this.npc.target].position.X-736-this.npc.width/2, (int) Main.player[this.npc.target].position.Y-16-this.npc.width/2, "Cursed Skull", 0);
				if (Random == 0) Paraspawn = NPC.NewNPC((int) Main.player[this.npc.target].position.X+700-this.npc.width/2, (int) Main.player[this.npc.target].position.Y-this.npc.width/2, "Cursed Skull", 0);
				if (Random == 5) Paraspawn = NPC.NewNPC((int) Main.player[this.npc.target].position.X-700-this.npc.width/2, (int) Main.player[this.npc.target].position.Y-this.npc.width/2, "Wraith", 0);
				if (Random == 5) Paraspawn = NPC.NewNPC((int) Main.player[this.npc.target].position.X+750-this.npc.width/2, (int) Main.player[this.npc.target].position.Y-this.npc.width/2, "Wraith", 0);
