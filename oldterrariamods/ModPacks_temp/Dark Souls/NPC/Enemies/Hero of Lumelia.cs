...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      // these are all the regular stuff you get , now lets see......
        ...
        if(!Corruption && AboveEarth && Main.rand.Next(2000)==1) return true;

	if(Main.hardMode && !Corruption && AboveEarth && Main.rand.Next(1000)==1) return true;

	if(Main.hardMode && Sky && Main.rand.Next(500)==1) return true;


	if(Main.hardMode && InGrayLayer && !Corruption && !Ocean && Main.rand.Next(1000)==1) return true;
