...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      if(Main.bloodMoon && !Dungeon && Main.hardMode && Main.rand.Next(120)==1) return true;

	if(Main.hardMode && Dungeon && Main.rand.Next(505)==1) return true;

	if(Main.bloodMoon && Dungeon && Main.hardMode && Main.rand.Next(10)==1) return true;

	if(Main.hardMode && InHell && Main.rand.Next(400)==1) return true;

	if(ModWorld.superHardmode && Dungeon && Main.rand.Next(100)==1) return true;
	
	if(ModWorld.superHardmode && AboveEarth && !Main.dayTime && Main.rand.Next(300)==1) return true;
...
public void DamagePlayer(Player player, ref int damage) //hook works!
{
	...
	{
				player.AddBuff(30, 300, false); //bleeding
                player.AddBuff(20, 300, false); //poisoned
				player.AddBuff("Broken Spirit", 1800, false); //no knockback resistance
    }
}
#endregion
...
public void PostDraw(SpriteBatch spriteBatch)
{
			...
			if(npc.spriteDirection == 1){
				...
				for(int i=1;i>-1;i--)
				{
					...
					spriteBatch.Draw(Main.goreTexture[Config.goreID["Ghost of the Darkmoon Knight Glow"]],
...
{
if (npc.life <= 0){
Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Darkmoon Knight Gore 1", 1f, -1);
Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Black Knight Gore 2", 1f, -1);
Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Black Knight Gore 3", 1f, -1);
Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Black Knight Gore 2", 1f, -1);
Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Black Knight Gore 3", 1f, -1);
}
}
