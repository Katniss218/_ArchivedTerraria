...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      if(Main.hardMode && InHell && Main.rand.Next(150)==1) return true;

	if(Main.hardMode && Jungle && !Corruption && InGrayLayer && Main.rand.Next(300)==1) return true;

	if(Main.hardMode && Dungeon && !Corruption && InGrayLayer && Main.rand.Next(300)==1) return true;

        if(!Main.hardMode && Dungeon && Main.rand.Next(700)==1) return true;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tonberry Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tonberry Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tonberry Gore 3", 1f, -1);
    }
}
#endregion