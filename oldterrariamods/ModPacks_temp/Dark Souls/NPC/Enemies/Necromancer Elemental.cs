...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
        if ((x < Main.maxTilesX*0.45f || x > Main.maxTilesX*0.75f) && Main.rand.Next(350)==1) return true;
	    return false;
    }
...
#region Projectiles
        ...
        if (customAi1 >= 10f)
        {
	        ...
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Chaos Elemental", 0);
