...
public bool SpawnNPC(int x,int y,int PID)
{
	...
	if(P.townNPCs > 0f) return false; 	

    if(AboveEarth && !Main.hardMode && !Main.dayTime && Main.rand.Next(35)==1) return true;

	if(AboveEarth && !Main.hardMode && Main.rand.Next(65)==1) return true;

	if(!Main.hardMode && !Meteor && !Dungeon && !Jungle && !Corruption && (InBrownLayer || InGrayLayer) && Main.rand.Next(40)==1) return true;

	if(!Main.hardMode && !Meteor && !Dungeon && !Jungle && !Corruption && (InBrownLayer || InGrayLayer) && !Main.dayTime && Main.rand.Next(30)==1) return true;

	if(!Main.hardMode && !Meteor && !Jungle && Corruption && (InBrownLayer || InGrayLayer) && Main.rand.Next(30)==1) return true;

	if(!Main.hardMode && Dungeon && InGrayLayer && Main.rand.Next(35)==1) return true;
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
			    if (Main.rand.Next(170)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        int type = Config.projectileID["Black Knights Spear"];//44;//0x37; //14;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Valkyrie Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Valkyrie Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Valkyrie Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Valkyrie Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Valkyrie Gore 3", 1f, -1);
    }
}
