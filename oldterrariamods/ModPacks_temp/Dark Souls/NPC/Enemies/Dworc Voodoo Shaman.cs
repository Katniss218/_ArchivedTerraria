...
public bool SpawnNPC(int x,int y,int PID)
{
	  ...
	  bool TropicalOcean = P.position.X < 3600;

      // these are all the regular stuff you get , now lets see......

      if(Main.hardMode && (Meteor || Jungle) && !Dungeon && !Corruption && AboveEarth && Main.rand.Next(450)==1) return true;

	if(Main.hardMode && (Meteor || Jungle) && !Dungeon && !Corruption && AboveEarth && !Main.dayTime && Main.rand.Next(130)==1) return true;

	if(Main.hardMode && (Meteor || Jungle) && !Dungeon && !Corruption && InGrayLayer && Main.rand.Next(120)==1) return true;

	if(Main.hardMode && TropicalOcean && Main.rand.Next(200)==1) return true;
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
			    if (Main.rand.Next(80)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        speedX *= num51;
				        speedY *= num51;
				        int damage = 65;//(int) (14f * npc.scale);
