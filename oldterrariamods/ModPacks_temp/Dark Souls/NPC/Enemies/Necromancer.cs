...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (Main.player[playerID].townNPCs > 0f || Main.player[playerID].zoneJungle || Main.player[playerID].zoneMeteor) return false;

	

	if (!Main.hardMode)
    {
        if (Main.player[playerID].zoneDungeon && Main.rand.Next(3000)==1) return true;
		if (oUnderworld && Main.rand.Next(500)==1) return true;
		if (oUnderworld && !Main.dayTime && Main.rand.Next(200)==1) return true;
        if ((x < Main.maxTilesX*0.35f || x > Main.maxTilesX*0.75f) && (oUnderground || oCavern) && Main.rand.Next(2000)==1) return true;
	    return false;
    }
	else if (Main.hardMode)
    {
		if (oUnderworld && Main.rand.Next(200)==1) return true;
		if (oUnderworld && !Main.dayTime && Main.rand.Next(100)==1) return true;
		if (Main.player[playerID].zoneDungeon && Main.rand.Next(200)==1) return true;
        if (Main.player[playerID].zoneHoly && (oUnderground || oCavern) && Main.rand.Next(150)==1) return true;
		if (Main.player[playerID].zoneEvil && (oUnderground || oCavern) && Main.rand.Next(300)==1) return true;
	    return false;
    }
    return false;
}
...
#region Projectiles
        ...
        if (customAi1 >= 10f)
        {
	        ...
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Armored Skeleton", 0);
		    ...
		    if (Main.rand.Next(120)==1)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
				    int damage = 70;
				    int type = Config.projectileID["Enemy Spell Sudden Death Strike"];//44;//0x37; //14;
				    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Necromancer Gore 1", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Necromancer Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Necromancer Gore 3", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Necromancer Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Necromancer Gore 3", 1.1f, -1);
    }
}
