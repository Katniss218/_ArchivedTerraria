...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (!Main.hardMode && Main.player[playerID].zoneJungle && (oSurface || oUnderSurface || oUnderground || oCavern))
    {
	    if (Main.rand.Next(12)==1) return true;
	    else if ((oUnderground || oCavern) && Main.rand.Next(9)==1) return true;	
        return false;
    }
    return false;
}
...
// set parameters
    ...
    float enrage_percentage=0.1f;  //  double movement speed below this life fraction. 0 for no enrage. Mummies enrage below .5
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dworc Gore 1", 1f, -1);
