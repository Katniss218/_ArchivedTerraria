	...
	if (npc.velocity.Y == 0f) // no jump/fall
	{
		...
		if (Main.rand.Next(90)==0) //1 in 90 chance boss will use attack when it flies down on top of you
                {
                   ...
                   if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
                   {
                           ...
                           int type = Config.projectileID["Anti Gravity Blast"];//44;//0x37; //14;
		...
		while (!flag7) // loop always ran full 100 time before I added "flag7 = true" below
		{
			...
			{ // (tp_x_target,m) is block under its feet I think
				if ((m < target_y_blockpos - 12 || m > target_y_blockpos + 12 || tp_x_target < target_x_blockpos - 12 || tp_x_target > target_x_blockpos + 12) && (m < y_blockpos - 5 || m > y_blockpos + 5 || tp_x_target < x_blockpos - 5 || tp_x_target > x_blockpos + 5) && Main.tile[tp_x_target, m].active)
