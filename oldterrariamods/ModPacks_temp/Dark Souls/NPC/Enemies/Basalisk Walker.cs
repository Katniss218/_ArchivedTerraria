...
public bool SpawnNPC(int x,int y,int PID)
{
	  ...
	  if(!Main.hardMode && !Dungeon && !Corruption && !Main.dayTime && AboveEarth && P.townNPCs <= 0f && ModWorld.Slayed.ContainsKey("Skeletron") && Main.rand.Next(20)==1) return true;
     	
      if(!Main.hardMode && Meteor && !Dungeon && !Corruption && !Main.dayTime && (AboveEarth || InBrownLayer || InGrayLayer) && Main.rand.Next(35)==1) return true;

      if(!Main.hardMode && Meteor && !Dungeon && !Corruption && Main.dayTime && (InBrownLayer || InGrayLayer || Ocean) && Main.rand.Next(120)==1) return true;

	  if(!Main.hardMode && Dungeon && !Main.dayTime && (InBrownLayer || InGrayLayer) && Main.rand.Next(170)==1) return true;

	  if(!Main.hardMode && InHell && Main.rand.Next(120)==1) return true;

      if(!Main.hardMode && Jungle && !Corruption && InGrayLayer && Main.rand.Next(650)==1) return true;

      if(!Main.hardMode && Corruption && !Main.dayTime && !Dungeon && InGrayLayer && Main.rand.Next(850)==1) return true;

      if(!Main.hardMode && (Meteor || Jungle) && !Dungeon && !Corruption && (AboveEarth || InBrownLayer || InGrayLayer) && Main.bloodMoon && Main.rand.Next(20)==1) return true;
...
// set parameters
    ...
    int boredom_time=120; // time until it stops targeting player if blocked etc, 60 for anything but chaos ele, 20 for chaos ele
	...
	if (Main.rand.Next(200)==1)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
				    int damage = 20;
				    int type = Config.projectileID["Enemy Bio Spit Ball"];//44;//0x37; //14;
				    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Parasite Zombie Gore 1", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Parasite Zombie Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Parasite Zombie Gore 3", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Parasite Zombie Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Parasite Zombie Gore 1", 1.1f, -1);
		Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Parasite Zombie Gore 3", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Parasite Zombie Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Parasite Zombie Gore 3", 1.1f, -1);
		Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Blood Splat", 1.1f, -1);
		Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Blood Splat", 1.1f, -1);
		Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Blood Splat", 1.1f, -1);
		Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Blood Splat", 1.1f, -1);
		Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Blood Splat", 1.1f, -1);
