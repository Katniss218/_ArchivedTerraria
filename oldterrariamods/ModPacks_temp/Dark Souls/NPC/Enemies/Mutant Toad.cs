public static bool SpawnNPC(int x, int y, int playerID)
{
	...
	{
	if (Main.npc[num36].active && Main.npc[num36].type == Config.npcDefs.byName["Mutant Toad"].type)
	{
	return false;
	}
	}
...
public void DamagePlayer(Player player, ref int damage) //hook works!
{
	...
	{
		player.AddBuff(20, 7200, false); //poisoned!
	}
...
{
    if (npc.life <= 0){
    Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mutant Toad Gore 1", 1f, -1);
    Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mutant Toad Gore 2", 1f, -1);
    Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mutant Toad Gore 2", 1f, -1);
    Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mutant Toad Gore 3", 1f, -1);
    Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mutant Toad Gore 3", 1f, -1);
    }
}