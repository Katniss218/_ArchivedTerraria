...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      if((Meteor || Jungle) && !Dungeon && !Corruption && AboveEarth && Main.rand.Next(400)==1) return true;

	if((Meteor || Jungle) && !Dungeon && !Corruption && InBrownLayer && Main.rand.Next(200)==1) return true;

	if((Meteor || Jungle) && !Dungeon && !Corruption && InGrayLayer && Main.dayTime && Main.rand.Next(150)==1) return true;

	if((Meteor || Jungle) && !Dungeon && !Corruption && InGrayLayer && !Main.dayTime && Main.rand.Next(100)==1) return true;


	if(Main.hardMode && Jungle && !Meteor && !Corruption && !Ocean && Main.rand.Next(2000)==1) return true;
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
			    ...
			    if (Main.rand.Next(220)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        int type = Config.projectileID["Enemy Spell Poison Storm Ball"];//44;//0x37; //14;
				        int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dworc Voodoomaster Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dworc Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dworc Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dworc Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dworc Gore 3", 1f, -1);
    }
}
