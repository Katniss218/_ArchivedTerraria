...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      // these are all the regular stuff you get , now lets see......
        ...
        if(Main.hardMode && !Main.dayTime && Jungle && AboveEarth && Main.rand.Next(35)==1) return true;

	if(Main.hardMode && !Main.dayTime && Dungeon && Main.rand.Next(25)==1) return true;

	if(Main.hardMode && !Main.dayTime && Jungle && InBrownLayer && Main.rand.Next(30)==1) return true;

        if(Main.hardMode && !Main.dayTime && Jungle && InGrayLayer && Main.rand.Next(20)==1) return true;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Hydromancer Gore 1", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Hydromancer Gore 2", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Hydromancer Gore 3", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Hydromancer Gore 2", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Hydromancer Gore 3", 1.2f, -1);
    }
}
