...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      if(!Meteor && !Dungeon && !Corruption && AboveEarth && Main.hardMode && !Main.dayTime && Main.rand.Next(6000)==1) return true;

	if(Main.hardMode && Meteor && !Corruption && InBrownLayer && Main.rand.Next(1650)==1) return true;

	if(Main.hardMode && !Main.dayTime && Dungeon && !Corruption && InGrayLayer && Main.rand.Next(3000)==1) return true;

	if(Main.hardMode && InHell && !Main.dayTime && Main.rand.Next(700)==1) return true;
