...
public bool SpawnNPC(int x,int y,int PID)
{
      ...
      if(Main.hardMode && !Main.dayTime && !Meteor && Jungle && !Dungeon && !Corruption && Main.rand.Next(1500)==1) 
      ...
      if(Main.hardMode && !Main.dayTime && !Corruption && !Ocean && AboveEarth && Main.rand.Next(3000)==1) 
      ...
      if(ModWorld.superHardmode && !Main.dayTime && !Corruption && !Ocean && AboveEarth && Main.rand.Next(70)==1) 
...
// set parameters
    ...
    int projectile_damage = 500;  //  projectile dmg: 35 for Skeleton Archer, 11 for Goblin Archer
    int projectile_id = Config.projectileID["Enemy Arrow of Bard"]; // projectile id: 82(Flaming Arrow) for Skeleton Archer, 81(Wooden Arrow) for Goblin Archer, 75(Happy Bomb) for Clown
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
			    if (Main.rand.Next(120)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        speedX *= num51;
				        speedY *= num51;
				        int damage = 500;//(int) (14f * npc.scale);
