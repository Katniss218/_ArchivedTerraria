...
public bool SpawnNPC(int x,int y,int PID)
{
	...
	if(ModWorld.superHardmode) return false;

        if(Main.hardMode && !Main.dayTime && Jungle && AboveEarth && Main.rand.Next(20005)==1) return true;

	if(Main.hardMode && Main.bloodMoon && Jungle && Main.rand.Next(12525)==1) return true;

	if(Main.hardMode && !Main.dayTime && InHell && Main.rand.Next(7030)==1) return true;

	if(Main.hardMode && Main.bloodMoon && Ocean && Main.rand.Next(8020)==1) return true;

        if(Main.hardMode && Main.bloodMoon && Sky && Main.rand.Next(8020)==1) return true;
