...
public static bool SpawnNPC(int x, int y, int PID)
{
	...
	if(ModWorld.superHardmode && Dungeon && Main.rand.Next(3000)==1) return true;
	
	if(ModWorld.superHardmode && AboveEarth && !Main.dayTime && Main.rand.Next(3000)==1) return true;

	if(ModWorld.superHardmode && !AboveEarth && Hallow && !Main.dayTime && Main.rand.Next(3000)==1) return true;

	for (int num36 = 0; num36 < 200; num36++)
	{
	    if (Main.npc[num36].active && Main.npc[num36].type == Config.npcDefs.byName["Dark Cloud"].type)
	    {
	        return false;
        }
    }
...
public void DamagePlayer(Player player, ref int damage) //hook works!
{
	...
	if (Main.rand.Next(4) == 0)
	{
				...
				player.AddBuff("Crippled", 1800, false); //you can barely jump and can not fly
...
if (Main.rand.Next(2) == 0)
	{
		    ...
		    player.AddBuff(36, 120, false); //broken armor
            player.AddBuff(24, 180, false); //on fire!
            player.AddBuff("Fracturing Armor", 3600, false); //defense goes time on every hit
		    player.AddBuff("Crippled", 600, false); //you can barely jump and can not fly
	}
...
#region Projectiles
    if(Main.netMode != 1)
    {
        ...
        if (customAi1 >= 10f)
        {
	        ...
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Crystal Knight II", 0);
	        ...
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Barrow Wight Nemesis", 0);
		        Main.npc[Spawned].velocity.Y = -8;
		        Main.npc[Spawned].velocity.X = Main.rand.Next(-10,10)/10;
		        npc.ai[0] = 20-Main.rand.Next(80);
                customspawn2 += 1f;
           ...
           if ((customspawn3 < 0) && Main.rand.Next(9950)==1)
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Assassin", 0);
		        Main.npc[Spawned].velocity.Y = -8;
		        Main.npc[Spawned].velocity.X = Main.rand.Next(-10,10)/10;
		        npc.ai[0] = 20-Main.rand.Next(80);
                customspawn3 += 1f;
		...
		if (Main.rand.Next(700)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
                    {
			            ...
			            speedX *= num51;
			            speedY *= num51;
			            int damage = 150;//(int) (14f * npc.scale);
		...
		if (Main.rand.Next(195)==1)
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
				    int damage = 92;
				    int type = Config.projectileID["Enemy Spell Abyss Poison Strike Ball"];//44;//0x37; //14;
				    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
 ...
 if (Main.rand.Next(120)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
                    {
			            ...
			            speedX *= num51;
			            speedY *= num51;
			            int damage = 135;//(int) (14f * npc.scale);
			            int type = Config.projectileID["Enemy Dragoon Lance"];//44;//0x37; //14;
			            int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
                        Main.projectile[num54].timeLeft = 700;
			            Main.projectile[num54].aiStyle=1;
                            Main.PlaySound(2, (int) npc.position.X, (int) npc.position.Y, 0x11);
 ...
 if (Main.rand.Next(300)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        speedX *= num51;
				        speedY *= num51;
				        int damage = 75;//(int) (14f * npc.scale);
				        int type = Config.projectileID["Enemy Spell Abyss Poison Strike Ball"];//44;//0x37; //14;
				        int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
...
if (Main.rand.Next(750)==1)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
				    int damage = 130;
				    int type = Config.projectileID["Enemy Spell Armageddon Blast Ball"];//44;//0x37; //14;
				    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
		    ...
		    if (Main.rand.Next(70)==1)
            {
			   float num48 = 14f;
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
				    int damage = 70; //(int) (14f * npc.scale);
				    int type = Config.projectileID["Enemy Spell Gravity 1 Ball"];//44;//0x37; //14;
				    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
		    ...
		    if (Main.rand.Next(280)==1)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedX *= num51;
				    speedY *= num51;
				    int damage = 105;//(int) (14f * npc.scale);
				    int type = Config.projectileID["Enemy Spell Abyss Poison Strike Ball"];//44;//0x37; //14;
				    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
 ...
 if (Main.rand.Next(526)==1) 
           {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
				    int damage = 80;//(int) (14f * npc.scale);
				    int type = Config.projectileID["Shadow Shot"];//44;//0x37; //14;
				    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
 ...
 if (Main.rand.Next(50)==1)
	    ...
	    if (Main.rand.Next(65)==1)
        {
		    ...
		    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
		    {
			    ...
			    int type = Config.projectileID["Enemy Arrow of Dark Cloud"]; //44;//0x37; //14;
			    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
    ...
    if (Main.rand.Next(555)==1)
        {
		    ...
		    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
		    {
			    ...
			    int type = Config.projectileID["Enemy Spell Abyss Storm Wave"]; //44;//0x37; //14;
			    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
			    Main.projectile[num54].timeLeft = 1300;
			    Main.projectile[num54].aiStyle=1;
                    Main.PlaySound(2, (int) npc.position.X, (int) npc.position.Y, 0x11);
