...
public bool SpawnNPC(int x,int y,int PID)
{
	...
	{
	    if (Main.npc[num36].active && Main.npc[num36].type == Config.npcDefs.byName["Slogra Boss"].type)
	    {
	        return false;
		}
	}

      if(Jungle && !Main.dayTime && !ModWorld.superHardmode && !Main.hardMode && AboveEarth && Main.rand.Next(6000)==1) 
	...
	if(Meteor && !Main.hardMode && !ModWorld.superHardmode && !Main.dayTime && InBrownLayer && Main.rand.Next(3000)==1) 
...
Color color = new Color();
	int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 18, npc.velocity.X, npc.velocity.Y, 200, color, 1f+comboDamage/40);
	Main.dust[dust].noGravity = true;
...
if(Main.netMode != 1)
{
	...
	//customAi5 += (Main.rand.Next(2, 5) * 0.1f) * npc.scale;
        if (customspawn1 == 0)
        {
		if ((customspawn1 < 1) && (comboDamage > 35) && Main.rand.Next(750)==1)
	        {
	...
	if (breakCombo == true || (enraged == true && Main.rand.Next(700)==1)) 
        {
...
public void NPCLoot()
{
	Gore.NewGore(npc.position,npc.velocity,"Slogra Gore 1",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Slogra Gore 2",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Slogra Gore 3",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Slogra Gore 2",0.9f,-1);
	Gore.NewGore(npc.position,npc.velocity,"Slogra Gore 3",0.9f,-1);
