...
public bool SpawnNPC(int x,int y,int PID)
{
    ...
    if(ModWorld.superHardmode && !Main.dayTime && ModWorld.Slayed.ContainsKey("Witchking") && !ModWorld.Slayed.ContainsKey("Abysmal Oolacile Sorcerer") && AboveEarth && Main.rand.Next(100)==1) return true;

	if(ModWorld.superHardmode && !Main.dayTime && ModWorld.Slayed.ContainsKey("Witchking") && !ModWorld.Slayed.ContainsKey("Abysmal Oolacile Sorcerer") && InBrownLayer && Main.rand.Next(100)==1) return true;

	if(ModWorld.superHardmode && Main.bloodMoon && ModWorld.Slayed.ContainsKey("Witchking") && !ModWorld.Slayed.ContainsKey("Abysmal Oolacile Sorcerer") && AboveEarth && Main.rand.Next(50)==1) return true;

	if(ModWorld.superHardmode && Main.bloodMoon && ModWorld.Slayed.ContainsKey("Witchking") && ModWorld.Slayed.ContainsKey("Abysmal Oolacile Sorcerer") && !ModWorld.Slayed.ContainsKey("Artorias") && AboveEarth && Main.rand.Next(2850)==1) return true;
...
if(Main.netMode != 1)
    {
        ...
        if (customAi5 >= 10f)
        {
	        ...
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Barrow Wight Phantom", 0);
           ...
           if ((customspawn3 < 1) && Main.rand.Next(2050)==1)
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Taurus Knight", 0);
		        Main.npc[Spawned].velocity.Y = -8;
		        Main.npc[Spawned].velocity.X = Main.rand.Next(-10,10)/10;
		        //npc.ai[5] = 20-Main.rand.Next(80);
                customspawn3 += 1f;
