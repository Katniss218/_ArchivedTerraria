...
public static bool SpawnNPC(int x,int y,int PID)
{
      ...
      // these are all the regular stuff you get , now lets see......
        ...
        //if(ModWorld.superHardmode && Main.rand.Next(4)==1) return true;

	if(ModWorld.superHardmode && (Sky || AboveEarth) && !ModWorld.Slayed.ContainsKey("Seath the Scaleless") && FrozenOcean && Main.rand.Next(100)==1) return true;

	if(Main.hardMode && P.townNPCs > 2f && ModWorld.Slayed.ContainsKey("Artorias") && !ModWorld.Slayed.ContainsKey("Seath the Scaleless") && !Main.dayTime && Main.rand.Next(1000)==1) 
...
public void AI()
{
	...
	if (!Main.player[npc.target].dead) // npc.target < 0 || npc.target == 255 ||
	{
	...
	if (Main.netMode != 1)
	{
		if (npc.type == Config.npcDefs.byName["Seath the Scaleless Head"].type && npc.ai[0] == 0f)
		{
			...
			{
				int num98 = Config.npcDefs.byName["Seath the Scaleless Body"].type;
				if (num97 == 1 || num97 == 18)
				{
					num98 = Config.npcDefs.byName["Seath the Scaleless Legs"].type;
				}
				else
				{
					if (num97 == 22)
					{
						num98 = Config.npcDefs.byName["Seath the Scaleless Body 2"].type;
					}
					else
					{
						if (num97 == 23)
						{
							num98 = Config.npcDefs.byName["Seath the Scaleless Body 3"].type;
						}
						else
						{
							if (num97 == 24)
							{
								num98 = Config.npcDefs.byName["Seath the Scaleless Tail"].type;
							}
						}
					}
				}
	...
	bool flag11 = false;
	if (npc.type >= Config.npcDefs.byName["Seath the Scaleless Body 2"].type && npc.type <= Config.npcDefs.byName["Seath the Scaleless Tail"].type)
	{
		flag11 = true;
	}
	if (!flag11)
	...
	if (npc.type == Config.npcDefs.byName["Seath the Scaleless Head"].type)
	{
		num115 = 11f;
		num116 = 0.25f;
	}
	...
	else
	{
		...
		{
			if (npc.type != Config.npcDefs.byName["Seath the Scaleless Head"].type && npc.type != 117 && npc.soundDelay == 0)
			{
			...
			num118 *= num125;
			num119 *= num125;
			if ((npc.type == 13 || npc.type == 7))
			{
				bool flag13 = true;
				for (int num126 = 0; num126 < 255; num126++)
				{
					if (Main.player[num126].active && !Main.player[num126].dead)
					{
						flag13 = false;
					}
				}
    ...
    if (breath) {
        ...
        //for (int pcy = 0; pcy < 10; pcy++) {
            Projectile.NewProjectile(npc.position.X + (float)npc.width / 2f, npc.position.Y + (float)npc.height / 2f, npc.velocity.X * 3f + (float)Main.rand.Next(-2, 3), npc.velocity.Y * 3f + (float)Main.rand.Next(-2, 3), Config.projDefs.byName["Frozen Dragon's Breath"].type, 65, 1.2f, 255);
    ...
    if (Main.rand.Next(420) == 0) {
        for (int pcy = 0; pcy < 10; pcy++) {
	        Projectile.NewProjectile((float)nT.position.X - 800 + Main.rand.Next(1600), (float)nT.position.Y - 500f, (float)(-40 + Main.rand.Next(80)) / 10, 10.9f, Config.projDefs.byName["Frozen Tear"].type, 87, 2f, 255); //9.9f was 14.9f
            Main.PlaySound(2, -1, -1, 20);

        }
    }
    if (Main.rand.Next(760) == 0) {
        for (int pcy = 0; pcy < 10; pcy++) {
	        Projectile.NewProjectile((float)nT.position.X - 500 + Main.rand.Next(1000), (float)nT.position.Y - 500f, (float)(-100 + Main.rand.Next(200)) / 10, 12.5f, Config.projDefs.byName["Dragon Meteor"].type, 100, 2f, 255); //9.5f was 14.9f
            Main.PlaySound(2, -1, -1, 20);
        }
    }
