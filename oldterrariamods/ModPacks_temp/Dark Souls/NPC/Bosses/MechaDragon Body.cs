...
int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 6, Main.rand.Next(-20,20)*2, Main.rand.Next(-20,20)*2, 100, color, 10f);
Main.dust[dust].noGravity = false;
dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 6, Main.rand.Next(-20,20)*2, Main.rand.Next(-20,20)*2, 100, color, 6f);
Main.dust[dust].noGravity = false;
...
Main.dust[dust].noGravity = false;
dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 6, 0, 0, 100, Color.White, 10.0f);
...
if (npc.position.X < Main.npc[(int)npc.ai[1]].position.X)
{
    ...
    if (Timer >= 600)
    {
        if (Main.netMode != 2)
        {
            ...
            int num54 = Projectile.NewProjectile(vector8.X, vector8.Y,(float)((Math.Cos(rotation) * num48)*-1),(float)((Math.Sin(rotation) * num48)*-1), "Crystal Fire", 20, 0f, 0);
...
{
    int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y-30), npc.width, npc.height, 6, 0, 0, 100, Color.White, 2.0f);
    Main.dust[dust].noGravity = true;
}
