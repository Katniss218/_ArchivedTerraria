...
#region Spawn
public static bool SpawnNPC(int x,int y,int PID)
{
	  ...
	  bool BeforeThree = x < Main.maxTilesX*0.3f; 
      // these are all the regular stuff you get , now lets see......
	  
	if(ModWorld.superHardmode && !BeforeThree && Main.bloodMoon && AboveEarth && Main.rand.Next(1000)==1) return true;

	if(ModWorld.superHardmode && BeforeThree && !ModWorld.Slayed.ContainsKey("Artorias") && Main.bloodMoon && AboveEarth && Main.rand.Next(30)==1) return true;

	if(ModWorld.superHardmode && BeforeThree && !ModWorld.Slayed.ContainsKey("Artorias") && Main.bloodMoon && Sky && Main.rand.Next(5)==1) return true;

	if(ModWorld.superHardmode && !Main.dayTime && Sky && Main.rand.Next(500)==1) return true;

	if(ModWorld.superHardmode && !Main.dayTime && Sky && Main.bloodMoon && Main.rand.Next(150)==1) return true;

	if(ModWorld.superHardmode && Main.bloodMoon && Sky && !ModWorld.Slayed.ContainsKey("Artorias") && Main.rand.Next(20)==1) return true;

	if(ModWorld.superHardmode && !Main.dayTime && InHell && Main.rand.Next(500)==1) return true;

	if(Main.hardMode && P.townNPCs > 2f && ModWorld.Slayed.ContainsKey("The Hunter") && !ModWorld.Slayed.ContainsKey("Hellkite Dragon") && !Main.dayTime && Main.rand.Next(200)==1) 
...
public void AI()
{
	...
	if (Main.netMode != 1)
	{
		if (npc.type == Config.npcDefs.byName["Hellkite Dragon Head"].type && npc.ai[0] == 0f)
		{
			...
			{
				int num98 = Config.npcDefs.byName["Hellkite Dragon Body"].type;
				if (num97 == 1 || num97 == 8)
				{
					num98 = Config.npcDefs.byName["Hellkite Dragon Legs"].type;
				}
				else
				{
					if (num97 == 11)
					{
						num98 = Config.npcDefs.byName["Hellkite Dragon Body 2"].type;
					}
					else
					{
						if (num97 == 12)
						{
							num98 = Config.npcDefs.byName["Hellkite Dragon Body 3"].type;
						}
						else
						{
							if (num97 == 13)
							{
								num98 = Config.npcDefs.byName["Hellkite Dragon Tail"].type;
							}
						}
					}
				}
	...
	bool flag11 = false;
	if (npc.type >= Config.npcDefs.byName["Hellkite Dragon Body 2"].type && npc.type <= Config.npcDefs.byName["Hellkite Dragon Tail"].type)
	{
		flag11 = true;
	}
	if (!flag11)
	...
	if (npc.type == Config.npcDefs.byName["Hellkite Dragon Head"].type)
	{
		num115 = 11f;
		num116 = 0.25f;
	}
	...
	else
	{
		...
		{
			if (npc.type != Config.npcDefs.byName["Hellkite Dragon Head"].type && npc.type != 117 && npc.soundDelay == 0)
			{
    ...
    if (breath) {
        ...
        //for (int pcy = 0; pcy < 10; pcy++) {
            Projectile.NewProjectile(npc.position.X + (float)npc.width / 2f, npc.position.Y + (float)npc.height / 2f, npc.velocity.X * 3f + (float)Main.rand.Next(-2, 3), npc.velocity.Y * 3f + (float)Main.rand.Next(-2, 3), Config.projDefs.byName["Dragon's Breath"].type, 65, 1.2f, 255);
        //}
        breathCD--;
        //}
    }
    ...
    if (Main.rand.Next(333) == 0) {
        for (int pcy = 0; pcy < 10; pcy++) {
	        Projectile.NewProjectile((float)nT.position.X - 600 + Main.rand.Next(1200), (float)nT.position.Y - 500f, (float)(-40 + Main.rand.Next(80)) / 10, 13.9f, Config.projDefs.byName["Flame Rain"].type, 54, 2f, 255);
            Main.PlaySound(2, -1, -1, 20);
        }
    }
    if (Main.rand.Next(660) == 0) {
        for (int pcy = 0; pcy < 10; pcy++) {
	        Projectile.NewProjectile((float)nT.position.X - 100 + Main.rand.Next(200), (float)nT.position.Y - 500f, (float)(-40 + Main.rand.Next(80)) / 10, 12.9f, Config.projDefs.byName["Dragon Meteor"].type, 65, 2f, 255);
            Main.PlaySound(2, -1, -1, 20);
        }
    }
...
if (npc.life <= 0){
Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Hellkite Dragon Head Gore", 1f, -1);
