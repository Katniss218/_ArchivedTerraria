...
public void AI()
{
	...
	if (left)
	{
	num54 = Projectile.NewProjectile(vector8.X, vector8.Y,-6+Main.rand.Next(-1,1), Main.rand.Next(-10,10)/5, "Crazed Orb", 52, 0f, 0);
	}
	else
	{
	num54 = Projectile.NewProjectile(vector8.X, vector8.Y,6+Main.rand.Next(-1,1), Main.rand.Next(-10,10)/5, "Crazed Orb", 52, 0f, 0);
	...
	{
		if (Main.projectile[lol].active && Main.projectile[lol].type == Config.projDefs.byName["Energy Pulse"].type)
		{
		subPhase = 0;
		break;
		}
	...
	{
	int num54 = Projectile.NewProjectile(vector8.X, vector8.Y,(float) Math.Sin(randomrot+((360/13)*(1+num36))*3),(float) Math.Cos(randomrot+((360/13)*(1+num36))*3), "Shadow Orb", 46, 0f, 0);
	}
	genericTimer2 = 0;
	}
	...
	{
	int num54 = Projectile.NewProjectile(vector8.X, vector8.Y,(float) Math.Sin(randomrot+((360/10)*(1+num36)))*6,(float) Math.Cos(randomrot+((360/10)*(1+num36)))*6, "Energy Pulse", 48, 0f, 0);
	...
	if (genericTimer2 >= 10)
	{
		...
		int num54 = Projectile.NewProjectile(vector8.X, vector8.Y,(float)((Math.Cos(rotation) * 0.5)*-1),(float)((Math.Sin(rotation) * 0.5)*-1), "Anti Matter Blast", 55, 0f, 0);
		genericTimer2 = 0;
	}
	}
	...
	if (genericTimer2 >= 8)
	{
		...
		rotation += Main.rand.Next(-50,50)/100;
		int num54 = Projectile.NewProjectile(vector8.X+Main.rand.Next(-100,100), vector8.Y+Main.rand.Next(-100,100),(float)((Math.Cos(rotation) * (0.5f+(Main.rand.Next(-3,3)/10)))*-1),(float)((Math.Sin(rotation) * (0.5f+(Main.rand.Next(-3,3)/10)))*-1), "Poison Smog", 34, 0f, 0);
		genericTimer2 = 0;
	}
	}
