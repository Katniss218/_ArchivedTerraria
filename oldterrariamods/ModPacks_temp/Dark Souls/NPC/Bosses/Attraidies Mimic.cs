...
public void AI()
{
	...
	{
	int num54 = Projectile.NewProjectile(vector8.X, vector8.Y,(float) Math.Sin(randomrot+((360/13)*(1+num36))*3),(float) Math.Cos(randomrot+((360/13)*(1+num36))*3), "Shadow Orb", 44, 0f, 0);
	}
	genericTimer2 = 0;
	}
	...
	if (genericTimer2 >= 20)
	{
		...
		int num54 = Projectile.NewProjectile(vector8.X, vector8.Y,(float)((Math.Cos(rotation) * 0.5)*-1),(float)((Math.Sin(rotation) * 0.5)*-1), "Anti Matter Blast", 40, 0f, 0);
		genericTimer2 = 0;
	}
	}
