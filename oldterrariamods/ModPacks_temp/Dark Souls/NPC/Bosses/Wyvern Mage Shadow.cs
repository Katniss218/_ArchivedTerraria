...
if (OptionSpawned == false)
	{
	OptionId = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Ghost Dragon Head", npc.whoAmI);
	...
	OptionSpawned = true;
        ...
        if (Main.netMode != 2)
        {
                if (npc.ai[0] >= 7 && npc.ai[2] < 3)
                {
                        ...
                        int damage = 70;
                        int type = Config.projectileID["Frozen Saw II"];
