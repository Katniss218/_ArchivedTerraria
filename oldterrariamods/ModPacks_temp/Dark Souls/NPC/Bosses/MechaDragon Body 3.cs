...
if (npc.position.X < Main.npc[(int)npc.ai[1]].position.X)
{
	...
	if (Timer >= 0)
	{
		if (Main.netMode != 2)
		{
			...
			int num54 = Projectile.NewProjectile(vector8.X, vector8.Y,(float)((Math.Cos(rotation) * num48)*-1),(float)((Math.Sin(rotation) * num48)*-1), "Crystal Fire", 15, 0f, 0);
...
{
	int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y+10), npc.width, npc.height, 6, 0, 0, 100, Color.White, 2.0f);
	Main.dust[dust].noGravity = true;
}
