...
int ticks=0;
static int type = Config.tileDefs.ID["Sandy Clay Pot"];
static int itemType = Config.itemDefs.byName["Cactus"].type; // what item to drop
int x, y;
...
public void KillTile(int x, int y, Player player)
{
	...
	if(Main.tile[x,y].frameNumber == 2 || Main.tile[x,y].frameNumber == 3)
	{
...
public void Update() 
{
	if(Main.tile[x, y].frameNumber < 4) 
	{
