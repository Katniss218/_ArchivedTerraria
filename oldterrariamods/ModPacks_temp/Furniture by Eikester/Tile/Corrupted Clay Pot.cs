...
int ticks=0;
static int type = Config.tileDefs.ID["Corrupted Clay Pot"];
static int itemType = Config.itemDefs.byName["Deathweed"].type; // what item to drop
static int itemType2 = Config.itemDefs.byName["Deathweed Seeds"].type; // what item to drop
int x, y;
...
{	
	if(Main.bloodMoon)
		...
		return Main.rand.Next(minTicks, maxTicks) + 2000;
}
