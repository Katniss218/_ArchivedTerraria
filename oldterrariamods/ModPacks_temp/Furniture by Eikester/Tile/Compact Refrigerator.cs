string name = "Compact Refrigerator";
string name2 = "Fridge";

public void PlaceTile(int x,int y,int pid)
{
    RealPos(ref x,ref y); //change x and y to top left corner of tile
    int ID = Chest.CreateChest(x, y); // you only create a chest
	
	if(Main.rand.Next(100) == 0)
	{
		Item item = new Item();
		item.SetDefaults(Config.itemDefs.byName["Archaeologist's Hat"].type, false);
		Main.chest[ID].item[0] = item;
	
		item = new Item();
		item.SetDefaults(Config.itemDefs.byName["Archaeologist's Jacket"].type, false);
		Main.chest[ID].item[1] = item;
	
		item = new Item();
		item.SetDefaults(Config.itemDefs.byName["Archaeologist's Pants"].type, false);
		Main.chest[ID].item[2] = item;
	}
}
public void MouseOverTile(int x,int y,Player P)
{
    ...
    P.showItemIcon2 = Config.itemDefs.byName[name].type;
}
...
public bool CanDestroyTile(int x,int y)
{
	...
	// is there another chest on top?
	if(Main.tileSolidTop[Main.tile[x,y].type])
	{
		for(int i = x; i < x + Config.tileDefs.width[Main.tile[x,y].type]; i++)
		{
			int x2 = i;
			int y2 = y-1;
			
			RealPos(ref x2, ref y2);
			
			if(Chest.FindChest(x2,y2)!=-1)
				return false;
		}
	}
