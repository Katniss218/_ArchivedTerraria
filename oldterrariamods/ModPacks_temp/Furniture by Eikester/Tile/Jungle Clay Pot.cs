...
int ticks=0;
static int type = Config.tileDefs.ID["Jungle Clay Pot"];
static int itemType = Config.itemDefs.byName["Moonglow"].type; // what item to drop
static int itemType2 = Config.itemDefs.byName["Moonglow Seeds"].type; // what item to drop
int x, y;
...
{	
	if(Main.tile[i,j].frameNumber == 3 && !Main.dayTime)
	{
		R = 0.1f; 
		G = 0.4f;
		B = 0.6f;
