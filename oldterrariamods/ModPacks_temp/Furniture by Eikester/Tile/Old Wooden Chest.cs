...
public void Update()
{
    ...
    if (chestindex != -1)
    {
        if (Chest.UsingChest(chestindex) != -1)
		{
			OpenChest(posX, posY);
		}
		else
		{
			CloseChest(posX, posY);
		}
	}
	else
	{
...
void OpenChest(int x, int y)
{
	...
	for(int i = 0; i < 2; i++)
	{
		for(int j = 0; j < 2; j++)
		{
			Main.tile[i + x, j + y].frameNumber = 1;
...
void CloseChest(int x, int y)
{
	...
	for(int i = 0; i < 2; i++)
	{
		for(int j = 0; j < 2; j++)
		{
			Main.tile[i + x, j + y].frameNumber = 0;
			Main.tile[i + x, j + y].frameX = (short)((offset + i) * 18);
			Main.tile[i + x, j + y].frameY = (short)(j * 18);
