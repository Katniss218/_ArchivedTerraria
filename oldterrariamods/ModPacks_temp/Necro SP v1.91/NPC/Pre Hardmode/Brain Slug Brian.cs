...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if ((x > Main.maxTilesX*0.1f && x < Main.maxTilesX*0.33f || x > Main.maxTilesX*0.66f && x < Main.maxTilesX*0.9f) && Main.rand.Next(20)==1) return true;
	    return false;
    }
    return false;
}
...
public void AI() {
    ...
    {
	int totalBrinSlugBrianspawn = 0;
	for (int num36 = 0; num36 < 200; num36++)
	{
	if (Main.npc[num36].active && Main.npc[num36].displayName == "Brain Slug Brain")
	{
	totalBrinSlugBrianspawn++;
	}
    }
    }
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Brain Slug Brian Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Brain Slug Brian Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Brain Slug Brian Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Brain Slug Brian Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Brain Slug Brian Gore 4", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Brain Slug Brian Gore 4", 1f, -1);
        }
}
#endregion