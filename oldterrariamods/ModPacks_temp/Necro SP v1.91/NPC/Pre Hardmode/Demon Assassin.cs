...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].zoneDungeon && !Main.player[playerID].zoneJungle && !Main.player[playerID].zoneMeteor && !oSky && !oSurface && !oUnderSurface)
    {
	    if (Main.player[playerID].townNPCs <= 0f && x < Main.maxTilesX*0.25f && Main.rand.Next(10)==1) return true;
        else if (Main.player[playerID].townNPCs <= 0f && (oMagmaCavern || oUnderworld) && Main.rand.Next(5)==1) return true;
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Demon Assasin Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Demon Assasin Gore 2", 1f, -1);
        }
}
#endregion