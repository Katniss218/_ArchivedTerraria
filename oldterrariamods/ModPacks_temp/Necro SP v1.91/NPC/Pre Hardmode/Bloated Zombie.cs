...
public static bool SpawnNPC(int x, int y, int playerID)
{
	if (Main.bloodMoon)
	{
		if (y < Main.rockLayer - 10 && y > (int)(Main.topWorld + 100f) && !Main.dayTime && !Main.player[playerID].zoneMeteor && Main.rand.Next(12)==1)
		{
			return true;
		}
		else return false;
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
	    ...
	    Gore.NewGore(npc.position,npc.velocity,"Bloated Zombie Gore 1",1.5f,-1);
	    Gore.NewGore(npc.position,npc.velocity,"Bloated Zombie Gore 2",1.5f,-1);
	    Gore.NewGore(npc.position,npc.velocity,"Bloated Zombie Gore 2",1.5f,-1);
	    Gore.NewGore(npc.position,npc.velocity,"Bloated Zombie Gore 3",1.5f,-1);
	    Gore.NewGore(npc.position,npc.velocity,"Bloated Zombie Gore 3",1.5f,-1);
}
#endregion