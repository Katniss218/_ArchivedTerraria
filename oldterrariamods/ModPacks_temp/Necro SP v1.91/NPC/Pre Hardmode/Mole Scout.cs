...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if ((x < Main.maxTilesX*0.25f || x > Main.maxTilesX*0.75f) && Main.rand.Next(10)==1) return true;
	    return false;
    }
    return false;
}
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mole Scout Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mole Scout Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mole Scout Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mole Scout Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mole Scout Gore 3", 1f, -1);
        }
}
