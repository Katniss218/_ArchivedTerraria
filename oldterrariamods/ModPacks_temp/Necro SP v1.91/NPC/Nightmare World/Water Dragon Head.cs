		...
		if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
        {
			if (Main.rand.Next(30)==1)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
				    int damage = 16;
				    int type = Config.projectileID["Water Shot"];//44;//0x37; //14;
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Water Dragon Gore 1", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Water Dragon Gore 2", 1f, -1);
        }
}
#endregion