...
public static bool SpawnNPC(int x, int y, int playerID)
{
    ...
    if (Main.hardMode && !Main.player[playerID].zoneJungle && Main.rand.Next(80)==1)
	{
	return true;
	}
	}
    ...
    if (customAi1 >= 10f)
    {
		...
		if (Main.rand.Next(40)==1)
        {
			...
			if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			{
				    ...
				    speedY *= num51;
				    int damage = 40;
				    int type = Config.projectileID["Enemy Lazor"];//44;//0x37; //14;
		...
		if (Main.rand.Next(200)==1)
        {
			...
			if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			{
				...
				int type = Config.projectileID["Enemy Emerald Chunk"];//44;//0x37; //14;
				int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
	{
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Weremole Gore 1", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Weremole Gore 2", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Weremole Gore 2", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Weremole Gore 3", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Weremole Gore 3", 1f, -1);
        }
}
#endregion