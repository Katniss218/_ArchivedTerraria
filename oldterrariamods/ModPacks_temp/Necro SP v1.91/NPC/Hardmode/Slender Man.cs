    ...
    if (customAi1 >= 10f)
    {
		...
		if (Main.rand.Next(75)==1)
        {
			...
			if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			{
				...
				speedY *= num51;
				int damage = 200;
				int type = Config.projectileID["Slender Ball"];//44;//0x37; //14;
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
		...
		int dust = Dust.NewDust(npc.position, rectangle.Width, rectangle.Height, 54, 0, 0, 50, Color.White, 4.0f);
