...
public static bool SpawnNPC(int x, int y, int playerID)
{
	...
	{
	if (Main.npc[num36].active && Main.npc[num36].type == Config.npcDefs.byName["Female Angel"].type)
	{
	return false;
	}
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Female Angel Gore 1", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Female Angel Gore 2", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Female Angel Gore 2", 1f, -1);
        }
}
#endregion