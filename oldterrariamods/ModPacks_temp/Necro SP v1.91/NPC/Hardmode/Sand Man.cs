...
public static bool SpawnNPC(int x, int y, int playerID)
{
	...
	{
	if (Main.hardMode && Main.sandTiles >= 20 && Main.rand.Next(4)==0)
	{
	return true;
	}
	}
...
if (npc.life <= 0){
Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sand Gore 1", 1f, -1);
Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sand Gore 2", 1f, -1);
Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sand Gore 2", 1f, -1);
}
}
#endregion