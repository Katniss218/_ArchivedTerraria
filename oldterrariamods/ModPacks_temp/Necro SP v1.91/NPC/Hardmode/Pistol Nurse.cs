...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
        if (Main.hardMode && Main.bloodMoon && Main.rand.Next(180)==1) return true;
    	if (Main.player[playerID].townNPCs <= 0f)
        {
            if ((x < Main.maxTilesX*0.32f || x > Main.maxTilesX*0.68f) && Main.hardMode && Main.rand.Next(900)==1) return true;
    	    ...
    	    else if ((oSurface || oUnderSurface || oUnderground || oCavern) && Main.hardMode && Main.rand.Next(900)==1) return true;
    	    return false;
        }
    }
		...
		if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
        {
			if (Main.rand.Next(30)==0)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
				    int damage = 80;
				    int type = Config.projectileID["Enemy Pistol Bullet"];//44;//0x37; //14;
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Nurse Gore 5", 1f, -1);
