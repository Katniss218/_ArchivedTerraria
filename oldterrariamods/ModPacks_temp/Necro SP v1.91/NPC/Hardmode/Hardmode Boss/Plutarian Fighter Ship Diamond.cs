...
public void AI()
{
    ...
    if (npc.ai[1] == 0) // First AI
    {
        ...
        int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 59, npc.velocity.X, npc.velocity.Y, 140, color, 2f);
        Main.dust[dust].noGravity = true;
	    }
	        else if (npc.life <= 1000)
	        {
	        Color color = new Color();
            int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 59, npc.velocity.X, npc.velocity.Y, 140, color, 2.5f);
            Main.dust[dust].noGravity = true;
	        }
#endregion
...
#region Projectile
        ...
        if (npc.ai[0] >= 140)
        {
            ...
            int RayShot1 = Projectile.NewProjectile(NPos.X, NPos.Y,(float)((Math.Cos(Angle) * 22f)*-1),(float)((Math.Sin(Angle) * 22f)*-1), "Energy Shot2", 50, 0f, 0);
    ...
    if (npc.ai[1] == 1) // Second AI
    {
        ...
        if(SweepStart) //If the sweep has begun
        {
            ...
            if (npc.ai[3] >= 10) //Do this when shot counter reaches max, increase this number to increase time between shots
            {
                ...
                int RayShot = Projectile.NewProjectile(NPos.X, NPos.Y,0f,12f, "Energy Shot2" , 50, 2f, 0);
...
NPC.NewNPC((int)npc.position.X,(int)npc.position.Y,"Genius Crystal Brain",0);
