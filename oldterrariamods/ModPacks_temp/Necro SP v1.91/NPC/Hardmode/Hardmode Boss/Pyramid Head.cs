...
public void AI() 
{
    ...
    if(npc.RunMethod("BarrowItemHit")) npc.defense = 1;
        ...
        int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 5, npc.velocity.X, npc.velocity.Y, 80, color, 2f);
        Main.dust[dust].noGravity = true;
	    }
	    else if (npc.life <= 6000)
	    {
	    Color color = new Color();
        int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 5, npc.velocity.X, npc.velocity.Y, 50, color, 3f);
        Main.dust[dust].noGravity = true;
	    }
#endregion
    ...
    if (customAi1 >= 10f)
    {
		...
		if (Main.rand.Next(75)==1)
        {
			...
			if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			{
				...
				int type = Config.projectileID["Blade Ball"];//44;//0x37; //14;
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
		...
		int dust = Dust.NewDust(npc.position, rectangle.Width, rectangle.Height, 5, 0, 0, 50, Color.White, 3.0f);
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Pyramid Head Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Pyramid Head Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Pyramid Head Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Pyramid Head Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Pyramid Head Gore 4", 1f, -1);
        }
}
#endregion