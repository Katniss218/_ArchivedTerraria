...
{				
									...
									if (Main.netMode != 1)
									{
										if (this.npc.type == Config.npcDefs.byName["Mechanical Seiryu Head"].type && npc.ai[0] == 0f)
										{
											...
											{
												int num98 = Config.npcDefs.byName["Mechanical Seiryu Body"].type;
												if (num97 == 4 || num97 == 9 || num97 == 14 || num97 == 19)
												{
													num98 = Config.npcDefs.byName["Mechanical Seiryu Legs"].type;
												}
												else
												{
													if (num97 == 21)
													{
														num98 = Config.npcDefs.byName["Mechanical Seiryu Body 2"].type;
													}
													else
													{
														if (num97 == 22)
														{
															num98 = Config.npcDefs.byName["Mechanical Seiryu Body 3"].type;
														}
														else
														{
															if (num97 == 23)
															{
																num98 = Config.npcDefs.byName["Mechanical Seiryu Tail"].type;
															}
														}
													}
												}
...
#region Projectile
    ...
    if (breath) 
    {
        ...
        Projectile.NewProjectile(npc.position.X + (float)npc.width / 2f, npc.position.Y + (float)npc.height / 2f, npc.velocity.X * 3f + (float)Main.rand.Next(-2, 3), npc.velocity.Y * 3f + (float)Main.rand.Next(-2, 3), Config.projDefs.byName["Breath Of Fire"].type, 75, 1.2f, 255);
        //}
        breathCD--;
        //}
    }
    ...
    if (Main.rand.Next(333) == 0) 
    {
        for (int pcy = 0; pcy < 10; pcy++) 
        {
	        ...
	        Projectile.NewProjectile((float)nT.position.X - 100 + Main.rand.Next(200), (float)nT.position.Y - 500f, (float)(-40 + Main.rand.Next(80)) / 10, 14.9f, Config.projDefs.byName["Flame Shower"].type, 64, 2f, 255);
            Main.PlaySound(26, -1, -1, 0);
	        customAi1 = 1f;
        }
    npc.netUpdate=true;
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mechanical Seiryu Gore 1", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mechanical Seiryu Gore 2", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mechanical Seiryu Gore 2", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mechanical Seiryu Gore 2", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mechanical Seiryu Gore 3", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Mechanical Seiryu Gore 4", 1f, -1);
        }
}
#endregion
