...
if (npc.velocity.Y < 4) npc.velocity.Y += 0.7f;
    ...
    int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 3, npc.velocity.X, npc.velocity.Y, 200, color, 2f);
    Main.dust[dust].noGravity = true;
	}
	else if (npc.life <= 4000)
	{
	Color color = new Color();
    int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 3, npc.velocity.X, npc.velocity.Y, 140, color, 4f);
    Main.dust[dust].noGravity = true;
	}
#endregion
...
int damage = 50;
int type = Config.projectileID["Flame"];
