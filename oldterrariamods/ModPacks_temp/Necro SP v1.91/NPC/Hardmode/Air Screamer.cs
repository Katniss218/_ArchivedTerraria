...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if (Main.hardMode && Main.player[playerID].position.X < ((Main.worldSurface * 150.0)) && Main.player[playerID].position.X > ((Main.worldSurface * 130.0)) && Main.rand.Next(40)==0) return true;
	    else if (Main.hardMode && Main.player[playerID].position.X > ((Main.worldSurface * 160.0)) && Main.player[playerID].position.X < ((Main.worldSurface * 175.0)) && Main.rand.Next(50)==0) return true;
        else if (Main.hardMode && Main.rand.Next(80)==1) return true;
	    return false;
    }
    return false;
}
    ...
    if (customAi1 >= 10f)
    {
		...
		if (Main.rand.Next(75)==1)
        {
			...
			if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			{
				...
				int type = Config.projectileID["Scream Ball"];//44;//0x37; //14;
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
	{
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Screamer Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Screamer Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Screamer Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Screamer Gore 3", 1f, -1);
        }
}
#endregion
