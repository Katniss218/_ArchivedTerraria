...
public void AI() {
{
	int totalRipperspawn = 0;
	for (int num36 = 0; num36 < 200; num36++)
	{
	if (Main.npc[num36].active && Main.npc[num36].displayName == "Ripper")
	{
	totalRipperspawn++;
	}
    }
    ...
    if (npc.ai[1] >= 10f)
    {
        ...
        if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
		{
			...
			if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			{
				...
				speedX *= num51;
				speedY *= num51;
				int damage = 28; //(int) (14f * npc.scale);
				int type = Config.projectileID["Enemy Dagger"];//44;//0x37; //14;
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
	{
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dagger Ripper Gore 1", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dagger Ripper Gore 2", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dagger Ripper Gore 2", 1f, -1);
        }
}
#endregion
