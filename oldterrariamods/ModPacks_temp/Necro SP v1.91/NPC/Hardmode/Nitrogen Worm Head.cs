...
public static bool SpawnNPC(int x, int y, int playerID) {
	if (Main.snowTiles >= 5 && Main.hardMode)
	{
	if(Main.rand.Next(40)==0) return true;
    return false;
    }
    return false;
...
public void AI()
{
	...
	for (int num36 = 0; num36 < 14; num36++)
	{
		...
		if (num36 >= 0 && num36 < 13)
		{
		lol = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Nitrogen Worm Body", npc.whoAmI);
		}
		else
		{
		lol = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Nitrogen Worm Tail", npc.whoAmI);
		}
		...
		if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
        {
			if (Main.rand.Next(30)==1)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
				    int damage = 30;
				    int type = Config.projectileID["Enemy Hail Shot"];//44;//0x37; //14;
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Nitrogen Worm Gore 1", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Nitrogen Worm Gore 2", 1f, -1);
        Gore.NewGore(vector8, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Nitrogen Worm Gore 3", 1f, -1);
        }
}
#endregion