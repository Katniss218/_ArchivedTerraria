...
Main.player[this.npc.target].statLife += 20;
Main.player[this.npc.target].HealEffect(20);
...
int style = Config.projDefs.byName["Boulder"].type;
int dmg = 60;
...
if(this.npc.displayName == "Cheeko"){
this.npc.scale += 0.2f;
this.npc.lifeMax += 25;
this.npc.life += 25;
}
...
public void NPCLoot()
{
	...
	for (int i = 1; i <= count; i++)
        ...
        if (npc.life <= 0)
        {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Chinchilla Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Chinchilla Gore 2", 1f, -1);
        }
}
#endregion
