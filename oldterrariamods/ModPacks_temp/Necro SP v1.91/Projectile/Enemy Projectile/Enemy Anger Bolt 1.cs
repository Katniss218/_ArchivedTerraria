...
public void AI()
{
    ...
    else
    {
	    ...
	    {
            projectile.Kill();
            Projectile.NewProjectile(projectile.position.X + (float)(projectile.width / 2), projectile.position.Y + (float)(projectile.height / 2), 0, 0, Config.projectileID["Enemy Anger Bolt 2"], 20, 8f, projectile.owner);
            return;
        }
    }
}
