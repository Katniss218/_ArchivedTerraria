	...
	if (Main.myPlayer == projectile.owner && projectile.ai[0] == 0f)
	{
		...
		else
		{
			if (projectile.ai[0] == 0f)
			{
				...
				if (num63 == 0f)
				{
                    Projectile.NewProjectile(projectile.position.X + (float)(projectile.width / 2), projectile.position.Y + (float)(projectile.height / 2), 0, 0, Config.projectileID["Enemy Electric Web"], 12, 8f, projectile.owner);
