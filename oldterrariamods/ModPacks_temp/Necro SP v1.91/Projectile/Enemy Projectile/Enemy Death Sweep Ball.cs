	...
	projectile.timeLeft = 0;
    {
		...
		for (int num40 = 0; num40 < 20; num40++)
		{
            if (projectile.position.X + (float)(projectile.width / 2) > Main.player[projectile.owner].position.X + (float)(Main.player[projectile.owner].width / 2))
            {
                ...
                Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*10f), projectile.position.Y + (float)(projectile.height-0.5f), -5, 0, Config.projectileID["Enemy Death Sweep Reverse"], 60, 8f, projectile.owner);
            }
            else
            {
                Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*-9), projectile.position.Y + (float)(projectile.height-0.5f), 5, 0, Config.projectileID["Enemy Death Sweep"], 60, 8f, projectile.owner);
            }
