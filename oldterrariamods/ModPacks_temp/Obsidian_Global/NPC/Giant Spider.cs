public static bool SpawnNPC(int x, int y, int playerID) {
	if (!Main.player[playerID].zoneJungle && !Main.player[playerID].zoneMeteor && Main.hardMode)
    {
	if (Main.player[playerID].position.X < ((Main.worldSurface * 80.0)) && Main.player[playerID].position.Y < ((Main.rockLayer * 35.0)) && Main.rand.Next(50)==1) return true;
	else if (Main.player[playerID].position.Y > ((Main.worldSurface * 25.0)) && Main.player[playerID].position.Y < ((Main.rockLayer * 35.0)) && Main.rand.Next(50)==1) return true;
	else if (Main.bloodMoon && Main.player[playerID].position.Y < ((Main.rockLayer * 35.0)) && Main.rand.Next(35)==1) return true;
	return false;
    }
return false;
}
...
float maxspeedX = 1.4f; // default : 1.1f
float accelerationX = 0.12f; //default : 0.07f
float longjumpX = 3.1f; // default : 1.5f
float longjumpY = 8f; // default : 8f
float jumpY = 6f; // default : 5f
float inertie = 0.8f; //default : 0.8f
