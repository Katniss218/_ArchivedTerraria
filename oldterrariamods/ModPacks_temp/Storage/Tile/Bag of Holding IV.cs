...
public class BagOfHolding : Interfaceable{
	...
	public bool DropSlot(int slot){
		...
		Item[] itemSlots = Config.tileInterface.itemSlots;
		ModPlayer.bank6[slot+offset] = itemSlots[slot];
		offset = 0;
		UpdateWriter();
		return false;
	}
	public static void UpdateWriter(){
		...
		for (int i = 0; i < 20; i++){
			...
			if (i+offset < ModPlayer.bank6.Length && i+offset >= 0){
				if (ModPlayer.bank6[i+offset].stack > 0){
					itemSlots[i] = ModPlayer.bank6[i+offset];
				}
			}
		}
	}
	public static void UpdateSaver(){
		...
		Item[] itemSlots = Config.tileInterface.itemSlots;
		for (int i = 0; i < 20; i++){
			ModPlayer.bank6[i+offset] = itemSlots[i];
		}
	}
}
