...
public class BagOfHolding : Interfaceable{
	...
	public bool DropSlot(int slot){
		...
		Item[] itemSlots = Config.tileInterface.itemSlots;
		ModPlayer.bank5[slot+offset] = itemSlots[slot];
		offset = 0;
		UpdateWriter();
		return false;
	}
	public static void UpdateWriter(){
		...
		for (int i = 0; i < 20; i++){
			...
			if (i+offset < ModPlayer.bank5.Length && i+offset >= 0){
				if (ModPlayer.bank5[i+offset].stack > 0){
					itemSlots[i] = ModPlayer.bank5[i+offset];
				}
			}
		}
	}
	public static void UpdateSaver(){
		...
		Item[] itemSlots = Config.tileInterface.itemSlots;
		for (int i = 0; i < 20; i++){
			ModPlayer.bank5[i+offset] = itemSlots[i];
		}
	}
}
