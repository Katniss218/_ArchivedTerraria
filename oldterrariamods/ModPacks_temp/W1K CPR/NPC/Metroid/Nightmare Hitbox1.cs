...
{
	spriteBatch.Draw(Main.blackTileTexture, new Vector2(npc.position.X - Main.screenPosition.X, npc.position.Y - Main.screenPosition.Y), new Rectangle(0,0,npc.width,npc.height),new Color(0,0,0,255));
}*/
...
/*public void DealtNPC(double damage, Player player)
{
	foreach(NPC npc2 in Main.npc)
	{
		if (npc2.whoAmI != npc.whoAmI)
		{
			...
			{
				npc2.immune[Main.myPlayer] = npc.immune[Main.myPlayer];
			}
