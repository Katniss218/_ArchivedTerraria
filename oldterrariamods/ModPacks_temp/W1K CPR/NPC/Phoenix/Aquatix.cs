...
public void AI()
{
	...
	if (npc.ai[0] > 600)
	{
		...
		{
		int dustExp = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 29, 0, 0, 100, Color.White, 3f);
		}
		TimerProjectile = -300;
		npc.ai[0] = 0;
		npc.life += 750;
...
{
if (npc.life <= 0){
Gore.NewGore(npc.position, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Water Phoenix GORE 1", 1f, -1);
Gore.NewGore(npc.position, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Water Phoenix GORE 2", 1f, -1);
Gore.NewGore(npc.position, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Water Phoenix GORE 3", 1f, -1);
Gore.NewGore(npc.position, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Water Phoenix GORE 2", 1f, -1);
Gore.NewGore(npc.position, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Water Phoenix GORE 3", 1f, -1);
}
}