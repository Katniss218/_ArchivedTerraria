...
public void AI()
{
	...
	if (npc.ai[0] > 600)
	{
		...
		{
		int dustExp = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 44, 0, 0, 100, Color.White, 3f);
		}
		TimerProjectile = -300;
		npc.ai[0] = 0;
		npc.life += 750;
    ...
    int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 44, npc.velocity.X, npc.velocity.Y, 200, color, 0.5f+npc.ai[0]/75);
	...
	{
	Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 44, 0, 0, 0, color, 3f);
	}
	}
	}
...
{
if (npc.life <= 0){
Gore.NewGore(npc.position, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Earth Phoenix GORE 1", 1f, -1);
Gore.NewGore(npc.position, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Earth Phoenix GORE 2", 1f, -1);
Gore.NewGore(npc.position, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Earth Phoenix GORE 3", 1f, -1);
Gore.NewGore(npc.position, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Earth Phoenix GORE 2", 1f, -1);
Gore.NewGore(npc.position, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Earth Phoenix GORE 3", 1f, -1);
}
}