public void AI()
{
	...
	{
    int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 65, Config.syncedRand.Next(-50,50)/10, Config.syncedRand.Next(-30,30)/10, 200, Color.White, 1f);
	...
	{
		if (Main.player[npc.target].buffType[num36] == Config.buffDefs.ID["Death Chao"])
		{
		buffOn = true;
