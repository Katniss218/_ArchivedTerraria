public static bool SpawnNPC(int x, int y, int playerID)
{
	...
	{
	if (Main.npc[num36].active && Main.npc[num36].type == Config.npcDefs.byName["Death"].type)
	{
	return false;
	}
	}
	...
	{
	if (Config.syncedRand.Next(150)==0)
	{
	return true;
	}
	}
...
public void AI()
{
    ...
    if ((npc.ai[1] >= 200 && npc.life > 2000) || (npc.ai[1] >= 120 && npc.life <= 2000))
    {
		...
		{
		Color color = new Color();
		int dust = Dust.NewDust(new Vector2((float) npc.position.X, (float) npc.position.Y), npc.width, npc.height, 54, npc.velocity.X+Config.syncedRand.Next(-10,10), npc.velocity.Y+Config.syncedRand.Next(-10,10), 200, color, 4f);
		Main.dust[dust].noGravity = true;
		}
...
Vector2 vector8 = new Vector2(npc.position.X + (npc.width * 0.5f), npc.position.Y + (npc.height / 2));
if (npc.life <= 0){
Gore.NewGore(vector8, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Death Gore 1", 1f, -1);
Gore.NewGore(vector8, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Death Gore 2", 1f, -1);
Gore.NewGore(vector8, new Vector2((float)Config.syncedRand.Next(-30, 31) * 0.2f, (float)Config.syncedRand.Next(-30, 31) * 0.2f), "Death Gore 3", 1f, -1);
