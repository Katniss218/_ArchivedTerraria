...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.bloodMoon && oSurface && Main.rand.Next(5000)==1) return true;
	else if (Main.player[playerID].townNPCs <= 0f && Main.hardMode && Main.player[playerID].zoneDungeon && !Main.player[playerID].zoneMeteor && (oSurface || oUnderSurface || oUnderground || oCavern))
    {
	    if ((x > Main.maxTilesX*0.2f && x < Main.maxTilesX*0.4f || x > Main.maxTilesX*0.6f && x < Main.maxTilesX*0.8f) && Main.rand.Next(500)==1) return true;
	    else if (Main.player[playerID].zoneDungeon && Main.rand.Next(200)==1) return true;
        return false;
    }
    return false;
}
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Projectile.NewProjectile(npc.position.X, npc.position.Y, 0, 0, "Smoke Field", 0, 0f, Main.myPlayer);
    }
}
#endregion