...
int drownTimer = 2600;
int drowningRisk = 2600;
//Spawns during Blood Moons, in the Dungeon, or in Hardmode from 2/10th to 4/10th and again from 6/10th to 8/10th (Width). Does not spawn if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (Main.player[playerID].townNPCs <= 0f && !Main.hardMode && Main.player[playerID].zoneDungeon && Main.rand.Next(200)==1) return true;
    else if (Main.bloodMoon && oSurface && Main.rand.Next(500)==1) return true;
	else if (Main.player[playerID].townNPCs <= 0f && Main.hardMode && !Main.player[playerID].zoneDungeon && !Main.player[playerID].zoneMeteor && (oSurface || oUnderSurface || oUnderground))
    {
	    if ((x > Main.maxTilesX*0.2f && x < Main.maxTilesX*0.4f || x > Main.maxTilesX*0.6f && x < Main.maxTilesX*0.8f) && Main.rand.Next(40)==1) return true;
	    else if (Main.player[playerID].zoneDungeon && Main.rand.Next(20)==1) return true;
        else if (Main.rand.Next(40)==1) return true;
        return false;
    }
    return false;
}
...
#region Barrow item Defense
public int BarrowItemHit() 
{ 
    return 1;
}
#endregion