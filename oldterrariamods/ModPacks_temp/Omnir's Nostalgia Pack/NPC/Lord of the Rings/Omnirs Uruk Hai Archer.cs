float customAi1;
int drownTimerMax = 2600;
int drownTimer = 2600;
int drowningRisk = 1000;
//Spawns in Hardmode, from the Surface into the Cavern. Spawns more beyond 6.5/10th (Width). Does not spawn in Dungeons, Meteor, or if there are more than 3 Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].townNPCs > 3 || !Main.hardMode || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
    if (oSurface || oUnderSurface || oUnderground)
    {
	    if (x > Main.maxTilesX*0.3f && x < Main.maxTilesX*0.7f && Main.rand.Next(10)==1) return true;
	    else if (x > Main.maxTilesX*0.7f && Main.rand.Next(20)==1) return true;
	    return false;
    }
    return false;
}
...
// set parameters
    ...
    int projectile_damage = 70;  //  projectile dmg: 35 for Skeleton Archer, 11 for Goblin Archer
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Uruk Hai Gore 1", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Uruk Hai Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Uruk Hai Gore 3", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Uruk Hai Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Uruk Hai Gore 3", 1.1f, -1);
    }
}
