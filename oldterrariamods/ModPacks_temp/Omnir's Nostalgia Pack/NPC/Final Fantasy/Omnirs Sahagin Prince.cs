...
//Spawns in big bodies of water, before the Cavern (Height), and before 2/10ths and after 8/10ths (Width) in Normal Mode, or anywhere (Width) in Hardmode. Does not spawn in Corruption, Dungeons, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].townNPCs > 0f || Main.tile[x,y-1].liquid < 20 || Main.player[playerID].zoneEvil || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
    if (!Main.hardMode && (oSurface || oUnderSurface || oUnderground))
    {
	    if ((x < Main.maxTilesX*0.2f || x > Main.maxTilesX*0.8f) && Main.rand.Next(50)==1) return true;
        if (Main.tile[x,y-1].liquid > 50 && Main.rand.Next(30)==1) return true;
	    return false;
    }
    if (Main.hardMode && (oSurface || oUnderSurface || oUnderground))
    {
	    if (Main.rand.Next(25)==1) return true;
        if (Main.tile[x,y-1].liquid > 50 && Main.rand.Next(3)==1) return true;
	    return false;
    }
    return false;
}
...
// set parameters
    ...
    float acceleration=.06f;  //  how fast it can speed up
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sahagin Prince Gore 1", 0.9f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sahagin Prince Gore 2", 0.9f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sahagin Prince Gore 3", 0.9f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sahagin Prince Gore 2", 0.9f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sahagin Prince Gore 3", 0.9f, -1);
    }
}
