...
//Spawns in the Cavern of the world.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (!Main.hardMode && (oCavern || oMagmaCavern) && Main.rand.Next(15)==1) return true;
	else if (Main.hardMode && oCavern && Main.rand.Next(40)==1) return true;
    return false;
}
#endregion
...
#region teleportation particle effects
	if (can_teleport)  //  chaos elemental type teleporter
	{
		if (npc.ai[3] == -120f)  //  boredom goes negative? I think this makes disappear/arrival effects after it just teleported
		{
			...
			{
				int num9 = Dust.NewDust(npc.position, npc.width, npc.height, 0, num6, num7, 200, default(Color), 2f);
			...
			{
				int num10 = Dust.NewDust(npc.oldPos[2], npc.width, npc.height, 0, -num6, -num7, 200, default(Color), 2f);
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Earth Elemental Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Earth Elemental Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Earth Elemental Gore 3", 1f, -1);
    }
}
#endregion