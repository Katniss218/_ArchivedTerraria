...
//Does not spawn with Town NPCs.  Spawns on the 7/10th to 9/10th part of the Surface (Width). Spawns more in the air/space. Does not spawn in Dungeons, Jungle, or Meteor.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (oSurface && (x > Main.maxTilesX*0.7f && x < Main.maxTilesX*0.9f) && Main.rand.Next(50)==1) return true;
    if (oSky && (x > Main.maxTilesX*0.7f && x < Main.maxTilesX*0.9f) && Main.rand.Next(12)==1) return true;
    return false;
}
#endregion
