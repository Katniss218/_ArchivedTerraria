...
//Spawns in big bodies of water, before the Cavern (Height), and before 3.8/10ths and after 6.2/10ths (Width). Does not spawn in Hardmode, Corruption, Dungeons, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].townNPCs > 0f || Main.tile[x,y-1].liquid < 10 || !Main.hardMode || Main.player[playerID].zoneEvil || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
    if (oSurface || oUnderSurface || oUnderground)
    {
	    if ((x < Main.maxTilesX*0.38f || x > Main.maxTilesX*0.62f) && Main.rand.Next(20)==1) return true;
        if (Main.tile[x,y-1].liquid > 50 && Main.rand.Next(2)==1) return true;
	    return false;
    }
    return false;
}
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sahagin Gore 1", 0.9f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sahagin Gore 2", 0.9f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sahagin Gore 3", 0.9f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sahagin Gore 2", 0.9f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Sahagin Gore 3", 0.9f, -1);
    }
}
