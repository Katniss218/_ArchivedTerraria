...
//Spawns only in hardmode, from the surface to the Underground, beyond 6.5/10ths of the map (Width).
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (!Main.hardMode || Main.player[playerID].townNPCs > 0f || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneJungle || Main.player[playerID].zoneMeteor) return false;
	if (oSurface || oUnderSurface || oUnderground)
    {
        if (x > Main.maxTilesX*0.65f && Main.rand.Next(50)==1) return true;
        return false;
    }
    return false;
}
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
		        if (Main.rand.Next(180)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        speedY *= num51;
				        int damage = 60;
				        int type = Config.projectileID["Enemy Spell Stench"];//44;//0x37; //14;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Eland Gore 1", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Eland Gore 2", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Eland Gore 3", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Eland Gore 2", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Eland Gore 3", 1.2f, -1);
    }
}
