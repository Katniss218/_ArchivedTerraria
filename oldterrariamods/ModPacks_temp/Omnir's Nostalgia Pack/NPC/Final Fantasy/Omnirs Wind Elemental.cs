...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (oSurface && (x > Main.maxTilesX*0.7f && x < Main.maxTilesX*0.9f) && Main.rand.Next(80)==1) return true;
    if (oSky && (x > Main.maxTilesX*0.7f && x < Main.maxTilesX*0.9f) && Main.rand.Next(20)==1) return true;
    return false;
}
#endregion
...
public void AI() 
{
    ...
    if (npc.wet)
    {
        int oldNPCLife=npc.life;
        if (customspawn1 < 1)
	    {
		    int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Omnirs Water Elemental", 0);
		    Main.npc[Spawned].velocity.Y = -8;
            Main.npc[Spawned].life=oldNPCLife;
