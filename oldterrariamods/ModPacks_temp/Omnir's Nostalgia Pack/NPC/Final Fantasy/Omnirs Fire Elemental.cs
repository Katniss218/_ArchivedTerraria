...
//Spawns in the Cavern (mostly with magma) and Underworld. Spawns less in Hardmode. Does not spawn in Dungeons, Jungle, Meteor, or if there are more than 2 Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if (Main.rand.Next(80)==1) return true;
        return false;
    }
    return false;
}
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Elemental Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Elemental Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Elemental Gore 3", 1f, -1);
    }
}
#endregion