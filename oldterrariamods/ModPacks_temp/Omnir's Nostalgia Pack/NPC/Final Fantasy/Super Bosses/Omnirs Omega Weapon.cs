...
public void AI() 
{
	...
	if (npc.position.Y > Main.player[npc.target].position.Y && (npc.position.Y - Main.player[npc.target].position.Y >= 300))
    {
        flyTimer = 200;
    }
    if (flyTimer > 0)
    ...
    if(Main.netMode != 1)
    {
        ...
        if (customAi1 >= 10f)
        {
            ...
            || (npc.position.Y < Main.player[npc.target].position.Y && (Main.player[npc.target].position.Y - npc.position.Y >= 1000))))
            {
                ...
                if (holdTimer <= 0)
                {
                    Main.NewText("Omega Weapon shields itself -- you're too far away!", 175, 75, 255);
                    holdTimer = 200;
                }
            }
            else
...
#region Charge
            ...
            if (Main.rand.Next(15)==1)
            {
    	        ...
    	        (float) Main.player[npc.target].position.X-100+Main.rand.Next(200),
    	        (float) Main.player[npc.target].position.Y-500.0f,
    	        (float) (-40+Main.rand.Next(80))/10,
    	        14.9f,
    	        Config.projectileID["Enemy Spell Meteor"], //Type
    	        (int) 100, //Damage
    	        2.0f,
    	        300
    	        );
