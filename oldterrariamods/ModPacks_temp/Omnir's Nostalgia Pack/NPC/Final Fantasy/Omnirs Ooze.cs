...
//Spawns fom the surface into the Cavern (Height), between 2.5/10th and 7.5/10th (Width). Does not spawn in Hardmode, Dungeons, Jungle, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.hardMode || Main.player[playerID].townNPCs > 0f || Main.player[playerID].zoneJungle || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
	if (oUnderSurface || oUnderground || oCavern)
    {
	    if (x > Main.maxTilesX*0.25f && x > Main.maxTilesX*0.75f && Main.rand.Next(30)==1) return true;
	    return false;
    }
    return false;
}
