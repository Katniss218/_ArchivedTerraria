float customAi1;
//Spawns only in Hardmode. On the surface, they spawn between 2/10th and 3/10th, and again between 7/10th and 8/10th (Width). They spawn throughout the Underground and Cavern (Width), too.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (x > Main.maxTilesX*0.2f && x < Main.maxTilesX*0.3f && Main.rand.Next(10)==1) return true;
	else if (x > Main.maxTilesX*0.7f && x < Main.maxTilesX*0.8f && Main.rand.Next(10)==1) return true;
	else if ((oCavern || oMagmaCavern) && Main.rand.Next(20)==1) return true;
	return false;
}
#endregion
...
public void AI() 
{
    ...
    if(Main.netMode != 1)
    {
        ...
        if (customAi1 >= 10f)
        {
            ...
            {
                if ((npc.life <= 499) && (npc.life >= 300)) 
                {
                    npc.scale = 1.2f;
                    npc.netUpdate=true;
                }
                if ((npc.life <= 299) && (npc.life >= 100)) 
                {
                    npc.scale = 1.5f;
                    npc.netUpdate=true;
                }
                if ((npc.life <= 99) && (npc.life >= 30)) 
                {
                    npc.scale = 1.7f;
                    npc.netUpdate=true;
                }
