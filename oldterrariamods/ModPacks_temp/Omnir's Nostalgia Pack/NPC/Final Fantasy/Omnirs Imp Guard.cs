//Spawns between 3/10th and 7/10th (Width) on the Surface. Does not spawn if player has more than 120 max health, in Dungeons, Jungle, Meteor, or if there are more than 2 Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.hardMode || Main.player[playerID].statLifeMax > 120 || Main.player[playerID].townNPCs > 2f || Main.player[playerID].zoneJungle || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
	if ((oSurface || oUnderSurface) && x > Main.maxTilesX*0.1f && x < Main.maxTilesX * 0.9f)
    {
	    if (Main.rand.Next(7)==1) return true;
	    return false;
    }
    return false;
}
...
// set parameters
    ...
    float enrage_top_speed=4;  //  faster when enraged, usually 2*top_speed
...
#region shoot and walk
	if (shoot_and_walk && Main.netMode != 1 && !Main.player[npc.target].dead) // can generalize this section to moving+projectile code
	{
		...
		npc.ai[2] += 1f; // throw countdown
        if (npc.ai[2] > shot_rate) // countdown expired, throw bomb
        { // for non bombs, may want to check facing, among other things
            Vector2 npc_center = new Vector2(npc.position.X + (float)npc.width * 0.5f - (float)(npc.direction * 24), npc.position.Y + 4f); // 24 behind center & 4 below top
            int projectile_handle = Projectile.NewProjectile(npc_center.X, npc_center.Y, (float)(3 * npc.direction), -5f, projectile_id, 0, 0f, Main.myPlayer); // toss bomb: starting velocity is 3 forward and 5 upward
            Main.projectile[projectile_handle].timeLeft = fuse_time; // set fuse
            npc.ai[2] = 0f; // and reset throw countdown
        }
	}
#endregion
//-------------------------------------------------------------------
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Imp Guard Gore 1", 0.9f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Imp Guard Gore 2", 0.9f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Imp Guard Gore 3", 0.9f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Imp Guard Gore 4", 0.9f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Imp Guard Gore 3", 0.9f, -1);
    }
}
#endregion