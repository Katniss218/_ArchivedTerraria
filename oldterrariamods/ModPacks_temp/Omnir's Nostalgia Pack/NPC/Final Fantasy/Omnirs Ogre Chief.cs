float customAi1;
int drownTimerMax = 2800;
int drownTimer = 2800;
int drowningRisk = 800;
//Spawns on the Surface, before 3/10ths and after 7/10ths (Width). Does not spawn in Hardmode, Dungeons, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].townNPCs > 0f || Main.hardMode || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
	if ((oSurface || oUnderSurface) && (x < Main.maxTilesX*0.3f || x > Main.maxTilesX*0.7f) && Main.rand.Next(50)==1) return true;
    return false;
}
#endregion
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ogre Chief Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ogre Chief Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ogre Chief Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ogre Chief Gore 4", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ogre Chief Gore 5", 1f, -1);
    }
}
#endregion
