...
public void AI() 
{
    ...
    if(Main.netMode != 1)
    {
        ...
        if (npc.ai[1] >= 10f)
        {
            ...
            || (npc.position.Y < Main.player[npc.target].position.Y && (Main.player[npc.target].position.Y - npc.position.Y >= 1000))))
            {
                ...
                if (holdTimer <= 0)
                {
                    Main.NewText("The Kraken is protected by the raging waters -- you're too far away!", 175, 75, 255);
                    holdTimer = 200;
                }
            }
            else
		    ...
		    if  (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
			    ...
			    if (Main.rand.Next(40)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        int type = Config.projectileID["Tentacle Swipe"];//44;//0x37; //14;
				        int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
				        Main.projectile[num54].timeLeft = 45;
				        Main.projectile[num54].aiStyle=1;
                          Main.PlaySound(2, (int) npc.position.X, (int) npc.position.Y, 0x11);
				        npc.ai[1] = 1f;
			        }
                ...
                if (Main.rand.Next(120)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        int type = Config.projectileID["Enemy Spell Ink"];//44;//0x37; //14;
				        int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
				        Main.projectile[num54].timeLeft = 200;
				        Main.projectile[num54].aiStyle=1;
                          Main.PlaySound(3, (int) npc.position.X, (int) npc.position.Y, 16);
				        npc.ai[1] = 1f;
			        }
	        	    npc.netUpdate=true;
                }
	...
	Lighting.addLight((int)npc.position.X / 16, (int)npc.position.Y / 16, 0.0f, 0.3f, 0.6f);
    return;
}
#endregion
