...
public void AI() 
{
    ...
    if(Main.netMode != 1)
    {
        ...
        if (npc.ai[1] >= 10f)
        {
            ...
            || (npc.position.Y < Main.player[npc.target].position.Y && (Main.player[npc.target].position.Y - npc.position.Y >= 1000))))
            {
                ...
                if (holdTimer <= 0)
                {
                    Main.NewText("The Lich is protected by the rotting earth -- you're too far away!", 175, 75, 255);
                    holdTimer = 200;
                }
            }
            else
		    ...
		    if (Main.rand.Next(150)==1)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
			        int damage = 40;
				    int type = Config.projectileID["Enemy Spell Ice 2 Ball"];//44;//0x37; //14;
		    ...
		    if (Main.rand.Next(150)==1)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
			        int damage = 40;
				    int type = Config.projectileID["Enemy Spell Lightning 2 Ball"];//44;//0x37; //14;
				    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
				    Main.projectile[num54].timeLeft = 100;
				    Main.projectile[num54].aiStyle=1;
                        Main.PlaySound(2, (int) npc.position.X, (int) npc.position.Y, 0x11);
	...
	Lighting.addLight((int)npc.position.X / 16, (int)npc.position.Y / 16, 0.1f, 0.6f, 0.0f);
    return;
}
#endregion
