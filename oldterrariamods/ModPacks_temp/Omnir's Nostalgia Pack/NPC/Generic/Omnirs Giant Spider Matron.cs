...
int drownTimer = 5000;
int drowningRisk = 300;
//Spawns on the Surface and Underground before 3.5/10th and after 6.5/10th (Width), and throughout the Cavern, also. Spawns a lot during Blood Moons. Does not spawn in the Jungle, Dungeons, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].zoneJungle || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
	if (Main.player[playerID].townNPCs <= 0f && !oUnderworld)
    {
	    if ((oCavern && x < Main.maxTilesX*0.35f || oCavern && x > Main.maxTilesX*0.65f) && Main.rand.Next(100)==1) return true;
	    else if (oCavern && Main.rand.Next(100)==1) return true;
	    else if (Main.bloodMoon && Main.rand.Next(20)==1) return true;
	    return false;
    }
    return false;
}
...
#region Projectiles
        ...
        if (customAi1 >= 10f)
        {
            ...
            if ((customspawn1 < 2) && Main.rand.Next(2)==1)
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Omnirs Poison Spider", 0);
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Giant Spider Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Giant Spider Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Giant Spider Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Giant Spider Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Giant Spider Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Giant Spider Gore 4", 1f, -1);
    }
}
#endregion