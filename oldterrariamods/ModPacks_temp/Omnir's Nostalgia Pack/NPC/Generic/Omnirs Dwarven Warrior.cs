...
//Spawns on the Surface and into the Cavern past 6.5/10th (Width). Does not spawn in the Jungle, Hardmode, Dungeons, Meteor, or if there are Town NPCs.
...
{    
    ...
    {
	    if (x > Main.maxTilesX*0.65f && Main.rand.Next(30)==1) return true;
	    else if (oUnderground && x > Main.maxTilesX*0.65f && Main.rand.Next(5)==1) return true;
	    return false;
    }
    return false;
}
