...
//Spawns on the Surface down into the Cavern in the Jungle. Spawns a lot during Blood Moons. Does not spawn if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (!Main.player[playerID].zoneJungle || oSky || oUnderworld) return false;
	if (Main.player[playerID].townNPCs <= 0f)
    {
	    if ((oSurface || oUnderSurface) && Main.rand.Next(300)==1) return true;
	    else if ((oUnderground|| oCavern) && Main.rand.Next(150)==1) return true;
	    return false;
    }
    else if (Main.bloodMoon && oCavern && Main.rand.Next(20)==1) return true;
    return false;
}
#endregion
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Giant Pelican Spider Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Giant Pelican Spider Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Giant Pelican Spider Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Giant Pelican Spider Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Giant Pelican Spider Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Giant Pelican Spider Gore 4", 1f, -1);
    }
}
#endregion