float customAi1;
float customspawn1;
int drownTimerMax = 2200;
int drownTimer = 2200;
int drowningRisk = 2200;
//Spawns before 3.5/10th and after 6.5/10th (Width) in the Cavern, or throughout the Cavern during a Blood Moon. Does not spawn in Hardmode, Dungeon, Meteor, if more than 2 Town NPCs are nearby.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].townNPCs > 2f || Main.hardMode || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
    if (oCavern)
    {
	    if ((x < Main.maxTilesX*0.35f || x > Main.maxTilesX*0.65f) && Main.rand.Next(80)==1) return true;
        else if (Main.bloodMoon && Main.rand.Next(15)==1) return true;
        return false;
    }
    return false;
}
...
// set parameters
    ...
    float enrage_top_speed=1.2f;  //  faster when enraged, usually 2*top_speed
...
#region Projectiles
        ...
        if (customAi1 >= 10f)
        {
            ...
            if ((customspawn1 < 4) && Main.rand.Next(300)==1)
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Omnirs Rotworm", 0);
...
#endregion
#region Charge
            if (npc.velocity.Y == 0f && Main.rand.Next(200)==1) 
            {
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Queen Rotworm Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Queen Rotworm Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Queen Rotworm Gore 3", 1f, -1);
    }
}
#endregion