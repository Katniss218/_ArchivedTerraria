...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (Main.player[playerID].townNPCs <= 0f && !Main.player[playerID].zoneDungeon && !Main.player[playerID].zoneMeteor && (oSurface || oUnderSurface || oUnderground))
    {
	    if ((x < Main.maxTilesX*0.35f || x > Main.maxTilesX*0.65f) && Main.rand.Next(60)==1) return true;
	    else if (Main.hardMode && Main.rand.Next(60)==1) return true;
        return false;
    }
    return false;
}
