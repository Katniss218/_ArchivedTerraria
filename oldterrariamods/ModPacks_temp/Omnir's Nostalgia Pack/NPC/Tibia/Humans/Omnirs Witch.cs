...
//Spawns on the Surface and into the Underground from 1/10th to 3/10th and again from 7/10th to 9/10th (Width). Does not spawn in the Dungeon, Hardmode, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if ((x > Main.maxTilesX*0.1f && x < Main.maxTilesX*0.3f || x > Main.maxTilesX*0.7f && x < Main.maxTilesX*0.9f) && Main.rand.Next(90)==1) return true;
	    return false;
    }
    return false;
}
...
// set parameters
    ...
    float top_speed=1.7f;  //  max walking speed, also affects jump length
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Witch Gore 1", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Witch Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Witch Gore 3", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Witch Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Witch Gore 3", 1.1f, -1);
    }
}
