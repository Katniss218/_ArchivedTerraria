...
//Spawns on the Surface down into the Cavern. Mostly spawns before 3.3/10th and after 6.6/10th (Width). Does not spawn in the Dungeon, Jungle, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if ((x < Main.maxTilesX * 0.33f || x > Main.maxTilesX*0.66f) && Main.rand.Next(40)==1) return true;
	    else if (oCavern && Main.rand.Next(20)==1) return true;
	    return false;
    }
    return false;
}
