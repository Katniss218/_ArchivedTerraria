...
//Spawns during the Waxing Crescent Moon, in the Hardmode Dungeon.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (NPC.AnyNPCs(Config.npcDefs.byName["Omnirs Ferumbras"].type) && Main.hardMode && Main.moonPhase == 5 && Main.player[playerID].townNPCs <= 0f && Main.player[playerID].zoneDungeon)
    {
...
// set parameters
    ...
    float enrage_top_speed=8f;  //  faster when enraged, usually 2*top_speed
...
#region Projectiles
        ...
        if (customAi1 >= 10f)
        {
            ...
            if ((customspawn1 < 3) && Main.rand.Next(200)==1)
	        {
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
		        if (Main.rand.Next(75)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        speedX *= num51;
				        speedY *= num51;
				        int damage = 55;//(int) (14f * npc.scale);
                ...
                if (Main.rand.Next(400)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
                    {
			            ...
			            int type = Config.projectileID["Enemy Spell Poison Storm"];//44;//0x37; //14;
			            int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
                        Main.projectile[num54].timeLeft = 100;
			            Main.projectile[num54].aiStyle=1;
                            Main.PlaySound(2, (int) npc.position.X, (int) npc.position.Y, 0x11);
