...
//Spawns in the Cavern, mostly before 2.3/10ths and after 7.7/10ths (Width). Does not Spawn in the Dungeon, Jungle, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (Main.player[playerID].townNPCs <= 0f && !Main.player[playerID].zoneDungeon && !Main.player[playerID].zoneJungle && !Main.player[playerID].zoneMeteor && !oUnderworld)
    {
	    if (oCavern && Main.rand.Next(400)==1) return true;
	    if ((x < Main.maxTilesX*0.23f || x > Main.maxTilesX*0.77f) && oCavern && Main.rand.Next(80)==1) return true;
        else if (Main.hardMode && oCavern && Main.rand.Next(25)==1) return true;
	    return false;
    }
    return false;
}
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
			    if (Main.rand.Next(30)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        int type = Config.projectileID["Heros Arrow"];//44;//0x37; //14;
