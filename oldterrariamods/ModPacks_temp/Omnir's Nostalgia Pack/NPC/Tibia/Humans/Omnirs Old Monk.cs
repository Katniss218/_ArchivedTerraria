...
//Spawns on the Surface and into the Underground from 1/10th to 4/10th and again from 6/10th to 9/10th (Width). Does not spawn in the Dungeon, Hardmode, Meteor, if the player does not have at least 140 health, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].statLifeMax <140 || Main.player[playerID].townNPCs > 0f || Main.hardMode || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
	if ((oSurface || oUnderSurface || oUnderground))
    {
	    if ((x > Main.maxTilesX*0.1f && x < Main.maxTilesX*0.4f || x > Main.maxTilesX*0.6f && x < Main.maxTilesX*0.9f) && Main.rand.Next(20)==1) return true;
	    return false;
    }
    return false;
}
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Old Monk Gore 1", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Old Monk Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Old Monk Gore 3", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Old Monk Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Old Monk Gore 3", 1.1f, -1);
    }
}
