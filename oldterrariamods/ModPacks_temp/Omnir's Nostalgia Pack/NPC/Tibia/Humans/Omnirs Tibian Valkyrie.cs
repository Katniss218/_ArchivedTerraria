...
//Spawns on the Surface and into the Underground from 1/10th to 3.3/10th and again from 6.6/10th to 9/10th (Width). Does not spawn in the Dungeon, Hardmode, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if ((x > Main.maxTilesX*0.1f && x < Main.maxTilesX*0.33f || x > Main.maxTilesX*0.66f && x < Main.maxTilesX*0.9f) && Main.rand.Next(40)==1) return true;
	    return false;
    }
    return false;
}
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
			    if (Main.rand.Next(50)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        speedX *= num51;
				        speedY *= num51;
				        int damage = 12;//(int) (14f * npc.scale);
