...
//Spawns in the Underground and Cavern before 2.5/10th and after 7.5/10th (Width). Does not Spawn in the Dungeon, Jungle, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (Main.player[playerID].townNPCs <= 0f && !Main.player[playerID].zoneDungeon && !Main.player[playerID].zoneJungle && !Main.player[playerID].zoneMeteor && (oSurface || oUnderSurface || oUnderground || oCavern))
    {
	    if ((x < Main.maxTilesX*0.25f || x > Main.maxTilesX*0.75f) && Main.rand.Next(110)==1) return true;
	    return false;
    }
    return false;
}
...
#region Projectiles
        ...
        if (customAi1 >= 10f)
        {
            ...
            if ((customspawn1 < 2) && Main.rand.Next(250)==1)
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Omnirs Rotting Ghoul", 0);
		        ...
		        if (Main.netMode == 2)
		        {
		            NetMessage.SendData(23, -1, -1, "Rotting Ghoul", Spawned, 0f, 0f, 0f, 0);
		        }
	        }
		    if (Main.rand.Next(45)==1)
	        {
			    ...
			    int type = Config.projectileID["Spell Effect Healing"];//44;//0x37; //14;
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))  
            {
	            ...
	            {
	                npc.ai[3] = 1;
	                Main.player[npc.target].statLife -= 8;
	            ...
	            {
	                npc.ai[3] = 1;
	                Main.player[npc.target].statMana -= 5;
                    if(Main.player[npc.target].statMana < 0) Main.player[npc.target].statMana = 0;
			        float num48 = 8f;
			        Vector2 vector8 = new Vector2(npc.position.X + (npc.width * 0.5f), npc.position.Y + (npc.height / 2));
			        float speedX = 0;
			        float speedY = 0;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Evil Priestess Gore 1", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Evil Priestess Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Evil Priestess Gore 3", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Evil Priestess Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Evil Priestess Gore 3", 1.1f, -1);
    }
}
