...
//Spawns in the Cavern, between 6/10ths and 9/10ths (Width). Does not Spawn in Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (NPC.AnyNPCs(Config.npcDefs.byName["Omnirs Black Knight"].type) || Main.player[playerID].zoneMeteor) return false;
	if (Main.player[playerID].townNPCs <= 0f && !Main.player[playerID].zoneMeteor && !oUnderworld)
    {
	    if ((x > Main.maxTilesX*0.6f || x < Main.maxTilesX*0.9f) && oCavern && Main.rand.Next(200)==1) return true;
	    return false;
    }
    return false;
}
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Black Knight Gore 1", 1f, -1);
