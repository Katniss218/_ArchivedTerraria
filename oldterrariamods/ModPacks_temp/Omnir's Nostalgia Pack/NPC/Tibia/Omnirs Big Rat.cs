...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if ((oSurface || oUnderSurface) && Main.rand.Next(3)==1) return true;
    return false;
}
#endregion
...
// set parameters
    ...
    float top_speed=1.0f;  //  max walking speed, also affects jump length
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Big Rat Gore 1", 1f, -1);
    }
}
#endregion
