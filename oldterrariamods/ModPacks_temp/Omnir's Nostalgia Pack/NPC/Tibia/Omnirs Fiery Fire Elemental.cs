float customAi1;
int drownTimerMax = 400;
int drownTimer = 400;
int drowningRisk = 400;
//Spawns in the lower Cavern and Underworld. Does not spawn in Hardmode, Dungeon, Jungle, if Town NPCs are nearby.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].townNPCs > 0f || Main.hardMode || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneJungle) return false;
	if ((oMagmaCavern || oUnderworld) && Main.rand.Next(5)==1) return true;
    return false;
}
#endregion
...
#region Too Big and Lava Jumping
    ...
    if (!npc.lavaWet && npc.wet) // Water death
	{
        Main.PlaySound(2, (int) npc.position.X, (int) npc.position.Y, 14);
        int num58 = Projectile.NewProjectile(this.npc.position.X,this.npc.position.Y,0,0,"Smoke Field",0,0f,Main.myPlayer);
        npc.netUpdate=true;
        npc.life = 0;
	}
#endregion
