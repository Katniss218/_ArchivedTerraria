...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (!Main.hardMode && !Main.player[playerID].zoneDungeon && !Main.player[playerID].zoneJungle && !Main.player[playerID].zoneMeteor && !oSky && !oSurface && !oUnderSurface)
    {
	    if (Main.player[playerID].townNPCs <= 0f && x < Main.maxTilesX*0.25f && Main.rand.Next(10)==1) return true;
        else if (Main.player[playerID].townNPCs <= 0f && !Main.hardMode && (oMagmaCavern || oUnderworld) && Main.rand.Next(5)==1) return true;
	    else if (Main.bloodMoon && Main.rand.Next(3)==1) return true;
	    return false;
    }
    return false;
}
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
		        if (Main.rand.Next(150)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        int type = Config.projectileID["Enemy Spell Explosion Ball"];//44;//0x37; //14;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Devil Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Devil Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Devil Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Devil Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Devil Gore 3", 1f, -1);
    }
}
