...
//Spawns in Hardmode, mostly in the Corruption. Does not spawn in Hallow, Jungle, Meteor, Dungeon, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (Main.hardMode && Main.player[playerID].townNPCs <= 0f && !Main.player[playerID].zoneHoly && !Main.player[playerID].zoneDungeon && !Main.player[playerID].zoneMeteor)
    {
	    if (Main.rand.Next(120)==1) return true;
	    return false;
    }
    else if (Main.hardMode && Main.player[playerID].townNPCs <= 0f && Main.player[playerID].zoneEvil && Main.rand.Next(30)==1) return true;
    return false;
}
#endregion
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
			    if (Main.rand.Next(90)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        int type = Config.projectileID["Enemy Throwing Knife"];//44;//0x37; //14;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dark Torturer Gore 1", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dark Torturer Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dark Torturer Gore 3", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dark Torturer Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Dark Torturer Gore 3", 1.1f, -1);
    }
}
