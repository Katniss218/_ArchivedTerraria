...
int drownTimer = 1800;
int drowningRisk = 0;
//Spawns on the Surface down into the Underground. Mostly spawns after 6/10th (Width). Does not spawn in the Dungeon, Hardmode, Jungle, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if (x > Main.maxTilesX*0.7f && Main.rand.Next(40)==1) return true;
	    else if ((oUnderSurface || oUnderground) && x > Main.maxTilesX*0.6f && Main.rand.Next(100)==1) return true;	
	    else if ((oUnderSurface || oUnderground) && Main.rand.Next(75)==1) return true;	
        return false;
    }
    return false;
}
