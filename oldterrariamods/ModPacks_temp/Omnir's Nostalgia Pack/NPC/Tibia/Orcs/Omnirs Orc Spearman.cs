...
//Spawns on the Surface down into the Underground. Mostly spawns after 4/10th (Width). Does not spawn in the Dungeon, Jungle, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if (x > Main.maxTilesX*0.57f && Main.rand.Next(20)==1) return true;
	    else if ((oUnderSurface || oUnderground) && x > Main.maxTilesX*0.4f && Main.rand.Next(20)==1) return true;	
	    else if ((oUnderSurface || oUnderground) && Main.rand.Next(30)==1) return true;	
        return false;
    }
    return false;
}
...
// set parameters
    ...
    float top_speed=1.1f;  //  max walking speed, also affects jump length
