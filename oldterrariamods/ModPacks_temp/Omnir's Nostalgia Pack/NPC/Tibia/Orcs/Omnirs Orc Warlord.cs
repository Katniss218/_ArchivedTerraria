...
//Spawns on the Surface down into the Underground. Mostly spawns after 6.8/10th (Width). Does not spawn in the Dungeon, Jungle, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].townNPCs > 0f || Main.player[playerID].zoneJungle || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
	if (!Main.hardMode && (oSurface || oUnderSurface || oUnderground) && (x > Main.maxTilesX*0.1f && x < Main.maxTilesX*0.9f))
    {
	    if (x > Main.maxTilesX*0.7f && Main.rand.Next(100)==1) return true;
	    else if ((oUnderSurface || oUnderground) && x > Main.maxTilesX*0.68f && Main.rand.Next(500)==1) return true;	
	    else if ((oUnderSurface || oUnderground) && Main.rand.Next(600)==1) return true;	
        return false;
    }
	if (Main.hardMode && (oSurface || oUnderSurface || oUnderground) && (x > Main.maxTilesX*0.1f && x < Main.maxTilesX*0.9f))
    {
        if ((oUnderSurface || oUnderground) && Main.rand.Next(60)==1) return true;	
        return false;
    }
    return false;
}
...
// set parameters
    ...
    float top_speed=2.3f;  //  max walking speed, also affects jump length
    ...
    float enrage_top_speed=4.6f;  //  faster when enraged, usually 2*top_speed
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
			    if (Main.rand.Next(50)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        int type = Config.projectileID["Enemy Throwing Star"];//44;//0x37; //14;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Orc Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Orc Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Orc Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Orc Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Orc Gore 3", 1f, -1);
    }
}
