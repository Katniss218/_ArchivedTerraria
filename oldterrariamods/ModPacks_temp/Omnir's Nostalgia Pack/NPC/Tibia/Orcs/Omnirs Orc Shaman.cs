...
//Spawns on the Surface down into the Underground. Mostly spawns after 5/10th (Width). Does not spawn in the Dungeon, Hardmode, Jungle, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if (x > Main.maxTilesX*0.7f && Main.rand.Next(50)==1) return true;
	    else if ((oUnderSurface || oUnderground) && x > Main.maxTilesX*0.5f && Main.rand.Next(12)==1) return true;	
	    else if ((oUnderSurface || oUnderground) && Main.rand.Next(60)==1) return true;	
        return false;
    }
    return false;
}
