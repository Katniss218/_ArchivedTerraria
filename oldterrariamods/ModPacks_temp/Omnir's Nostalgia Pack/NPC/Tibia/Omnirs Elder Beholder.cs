float customAi1;
float customspawn1;
//Spawns in only in hardmode: Cavern, mostly in Corruption; or anywhere during a Blood Moon. Does not spawn in the Jungle, Meteor or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].townNPCs > 0f || !Main.hardMode || Main.player[playerID].zoneJungle || Main.player[playerID].zoneMeteor) return false;
	if (Main.player[playerID].zoneEvil && Main.rand.Next(50)==1) return true;
	else if ((oSurface || oUnderSurface || oUnderground || oCavern) && Main.rand.Next(200)==1) return true;
    else if (Main.bloodMoon && Main.rand.Next(60)==1) return true;
    return false;
}
#endregion 
...
public void AI() 
{
    ...
    if(Main.netMode != 1)
    {
        ...
        if (customAi1 >= 10f)
        {
            ...
            if ((customspawn1 < 6) && Main.rand.Next(500)==1)
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Omnirs Crypt Shambler", 0);
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Elder Beholder Gore 1", 1.3f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Elder Beholder Gore 1", 1.3f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Elder Beholder Gore 1", 1.3f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Elder Beholder Gore 1", 1.3f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Elder Beholder Gore 2", 1.3f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Elder Beholder Gore 3", 1.3f, -1);
    }
}
#endregion
