...
//Spawns on the Surface and Underground in the Snow. Does not spawn in Hardmode, Jungle, Dungeon, Meteor, if Town NPCs are nearby, or if Player reaches more than 200 health.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.snowTiles <= 12 || Main.player[playerID].statLifeMax > 200 || Main.player[playerID].townNPCs > 0f || Main.hardMode || Main.player[playerID].zoneJungle || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
    if (Main.player[playerID].statLifeMax > 140 || Main.player[playerID].townNPCs > 0f || Main.hardMode || Main.player[playerID].zoneJungle || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
	if ((oSurface || oUnderSurface) && Main.rand.Next(2)==1) return true;
    return false;
}
#endregion
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Frost Troll Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Frost Troll Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Frost Troll Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Frost Troll Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Frost Troll Gore 3", 1f, -1);
    }
}
