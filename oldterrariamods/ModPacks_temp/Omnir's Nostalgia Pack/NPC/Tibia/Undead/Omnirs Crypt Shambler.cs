...
//Spawns in the Underground down into the Cavern. Mostly spawns between 1/10th to 3/10th and again from 7/10th to 9/10th (Width). Does not spawn in Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if ((oUnderSurface || oUnderground || oCavern) && (x > Main.maxTilesX*0.1f && x < Main.maxTilesX*0.9f))
    {
	    if ((x < Main.maxTilesX*0.3f || x > Main.maxTilesX*0.7f) && Main.rand.Next(80)==1) return true;
	    else if (Main.rand.Next(300)==1) return true;	
        if (Main.hardMode)
        {
 	        if (Main.rand.Next(8)==1) return true;
	        else if ((oUnderground || oCavern) && Main.rand.Next(10)==1) return true;	
            return false;           
        }
        return false;
    }
    if (Main.player[playerID].zoneJungle && Main.rand.Next(250)==1) return true;
    return false;
}
#endregion
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Crypt Shambler Gore 1", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Crypt Shambler Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Crypt Shambler Gore 3", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Crypt Shambler Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Crypt Shambler Gore 3", 1.1f, -1);
    }
}
