float customAi1;
float customspawn1;
//Spawns in Hardmode Corruption when there are no Town NPCs nearby, or if there is a Blood Moon
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (Main.hardMode && Main.player[playerID].zoneEvil)
    {
	    if (Main.player[playerID].townNPCs <= 0f && Main.rand.Next(200)==1) return true;
	    else if (Main.bloodMoon && Main.rand.Next(80)==1) return true;
    	return false;
    }
return false;
}
...
public void AI() 
{
    ...
    if(Main.netMode != 1)
    {
        ...
        if (customAi1 >= 10f)
        {
	        ...
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Omnirs Green Djinni", 0);
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
                ...
                if (Main.rand.Next(400)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        speedX *= num51;
				        speedY *= num51;
				        int damage = 1;//(int) (14f * npc.scale);
				        int type = Config.projectileID["Enemy Spell Drunkeness"];//44;//0x37; //14;
				        int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
				        Main.projectile[num54].timeLeft = 100;
				        Main.projectile[num54].aiStyle=23;
                            Main.PlaySound(2, (int) npc.position.X, (int) npc.position.Y, 0x11);
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Efreet Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Green Djinn Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Green Djinn Gore 3", 1f, -1);
    }
}
#endregion
