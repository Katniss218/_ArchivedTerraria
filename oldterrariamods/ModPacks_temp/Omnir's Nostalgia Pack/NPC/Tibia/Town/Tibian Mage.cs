...
public static void SetupShop(Chest chest) 
{
	...
	index++;
	chest.item[index].SetDefaults("Fire Field Rune");
	index++;
	chest.item[index].SetDefaults("Poison Field Rune");
	index++;
	chest.item[index].SetDefaults("Energy Field Rune");
	index++;
	chest.item[index].SetDefaults("Explosion Rune");
	index++;
    if (NPC.downedBoss3)
    {
