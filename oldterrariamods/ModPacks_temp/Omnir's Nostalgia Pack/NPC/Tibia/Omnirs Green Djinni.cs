...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if (Main.player[playerID].townNPCs <= 0f && Main.rand.Next(100)==1) return true;
	    else if (Main.bloodMoon && Main.rand.Next(50)==1) return true;
    	return false;
    }
return false;
}
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Green Djinn Gore 1", 1f, -1);
