...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.tile[x,y-1].liquid < 100 || Main.player[playerID].townNPCs > 0f || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
	if ((oSurface || oUnderSurface || oUnderground) && (x < Main.maxTilesX*0.1f || x > Main.maxTilesX*0.9f))
    {
	    if (Main.rand.Next(50)==1) return true;
	    else if ((oUnderSurface || oUnderground) && Main.rand.Next(70)==1) return true;	
        if (Main.hardMode)
        {
 	        if (Main.rand.Next(8)==1) return true;
	        else if ((oUnderSurface || oUnderground) && Main.rand.Next(10)==1) return true;	
            return false;           
        }
        return false;
    }
