...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if ((oSurface || oUnderSurface || oUnderground) && (x < Main.maxTilesX*0.1f || x > Main.maxTilesX*0.9f))
    {
	    ...
	    else if ((oUnderSurface || oUnderground) && Main.rand.Next(50)==1) return true;	
        if (Main.hardMode)
        {
 	        if (Main.rand.Next(8)==1) return true;
	        else if ((oUnderSurface || oUnderground) && Main.rand.Next(9)==1) return true;	
            return false;           
        }
        return false;
    }
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Predator Gore 1", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Predator Gore 2", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Predator Gore 3", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Predator Gore 2", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Predator Gore 3", 1.2f, -1);
    }
}
