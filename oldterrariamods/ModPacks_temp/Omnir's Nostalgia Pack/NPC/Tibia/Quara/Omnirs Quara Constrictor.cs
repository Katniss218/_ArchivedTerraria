...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if ((oSurface || oUnderSurface || oUnderground) && (x < Main.maxTilesX*0.1f || x > Main.maxTilesX*0.9f))
    {
        ...
        if (Main.hardMode)
        {
	        ...
	        else if ((oUnderSurface || oUnderground) && Main.rand.Next(6)==1) return true;	
            return false;           
        }
        return false;
    }
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Constrictor Gore 1", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Constrictor Gore 2", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Constrictor Gore 3", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Constrictor Gore 2", 1.2f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Quara Constrictor Gore 3", 1.2f, -1);
    }
}
