...
//Spawns on the Surface(before 3.8/10th and after 6.2/10th (Width)), and throughout the Underground and Cavern. Does not spawn in Hardmode, Dungeon, Meteor, if Town NPCs are nearby, or if Player reaches more than 200 health.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].statLifeMax >200 || Main.player[playerID].townNPCs > 0f || Main.hardMode || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
    else if ((oUnderSurface|| oUnderground || oCavern) && Main.rand.Next(20)==1) return true;
    return false;
}
#endregion
...
// set parameters
    ...
    float top_speed=0.8f;  //  max walking speed, also affects jump length
    ...
    float enrage_top_speed=1.6f;  //  faster when enraged, usually 2*top_speed
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Rotworm Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Rotworm Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Rotworm Gore 3", 1f, -1);
    }
}
#endregion