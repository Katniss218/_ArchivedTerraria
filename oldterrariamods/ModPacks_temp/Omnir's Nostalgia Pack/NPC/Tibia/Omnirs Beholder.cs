float customAi1;
float customspawn1;
//Spawns in only the Cavern, mostly in Corruption; or anywhere during a Blood Moon. Does not spawn in Hardmode, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].townNPCs > 0f || Main.hardMode || Main.player[playerID].zoneMeteor) return false;
	if (Main.player[playerID].zoneEvil && (oSurface || oUnderSurface || oUnderground || oCavern) && Main.rand.Next(30)==1) return true;
	else if ((oSurface || oUnderSurface || oUnderground || oCavern) && Main.rand.Next(400)==1) return true;
    else if (Main.player[playerID].statLifeMax >= 200 && (oSurface || oUnderSurface || oUnderground || oCavern) && Main.rand.Next(25)==1) return true;
    else if (Main.bloodMoon && Main.rand.Next(30)==1) return true;
    return false;
}
#endregion 
...
public void AI() 
{
    ...
    if(Main.netMode != 1)
    {
        ...
        if (customAi1 >= 10f)
        {
	        ...
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Skeleton", 0);
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Beholder Gore 1", 1.3f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Beholder Gore 1", 1.3f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Beholder Gore 1", 1.3f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Beholder Gore 1", 1.3f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Beholder Gore 2", 1.3f, -1);
    }
}
#endregion
