...
//Spawns on the Surface down into the Cavern. Mostly spawns before 6.8/10th (Width). Does not spawn in the Dungeon, Hardmode, Jungle, Meteor, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if (x < Main.maxTilesX*0.4f && Main.rand.Next(15)==1) return true;
	    else if (oCavern && x < Main.maxTilesX*0.65f && Main.rand.Next(6)==1) return true;	
	    else if (oCavern && Main.rand.Next(30)==1) return true;	
        return false;
    }
    return false;
}
...
// set parameters
    ...
    float top_speed=1.6f;  //  max walking speed, also affects jump length
    ...
    float enrage_top_speed=3.2f;  //  faster when enraged, usually 2*top_speed
