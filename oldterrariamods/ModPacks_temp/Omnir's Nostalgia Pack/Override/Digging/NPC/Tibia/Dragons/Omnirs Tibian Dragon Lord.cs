...
int num95 = 0;
int noJump = 0;
int drownTimerMax = 4500;
int drownTimer = 4500;
int drowningRisk = 2000;
//Spawns in the Underground into the Cavern, mostly before 3.2/10th and after 6.8/10th (Width) if no Town NPCs. Also spawns during Blood Moons, and will not spawn in the Jungle, Dundeon, or Meteor.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
        if (Main.bloodMoon && Main.rand.Next(300)==1) return true;
    	if (Main.player[playerID].townNPCs <= 0f)
        {
            if ((x < Main.maxTilesX*0.32f || x > Main.maxTilesX*0.68f) && Main.rand.Next(900)==1) return true;
            else if (Main.hardMode && Main.rand.Next(120)==1) return true;
    	    else if (oCavern && (x < Main.maxTilesX*0.32f || x > Main.maxTilesX*0.68f) && Main.rand.Next(150)==1) return true;
    	    else if ((oSurface || oUnderSurface || oUnderground || oCavern) && Main.rand.Next(900)==1) return true;
    	    return false;
        }
    }
    else if (Main.player[playerID].townNPCs <= 0f && Main.hardMode && Main.player[playerID].zoneEvil && Main.rand.Next(90)==1) return true;
    return false;
}
#endregion
...
// set parameters
    ...
    float top_speed=2.5f;  //  max walking speed, also affects jump length
        ...
        if (customAi1 >= 10f)
        {
		    ...
		    if (Main.rand.Next(300)==1)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    speedY *= num51;
				    int damage = 30;
				    int type = Config.projectileID["Enemy Spell Great Fire Wave Ball"];//44;//0x37; //14;
				    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
				    Main.projectile[num54].timeLeft = 0;
				    Main.projectile[num54].aiStyle=1;
                        Main.PlaySound(2, (int) npc.position.X, (int) npc.position.Y, 0x11);
...
#region Fly Through Walls
		...
		if ((!Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height)))
		{
			if ((npc.collideY) || (npc.collideX))
			{
                ...
                else if ((movedTowards >= 5f) &&  (movedTowards <= 9f))
                {
                    npc.velocity.Y = 0f;
                    npc.velocity.X = 0f;
                    movedTowards += 1;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Dragon Lord Gore 1", 1.15f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Dragon Lord Gore 2", 1.15f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Dragon Lord Gore 3", 1.15f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Dragon Lord Gore 2", 1.15f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Dragon Lord Gore 3", 1.15f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Dragon Lord Gore 4", 1.15f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Tibian Dragon Lord Gore 5", 1.15f, -1);
    }
}
#endregion