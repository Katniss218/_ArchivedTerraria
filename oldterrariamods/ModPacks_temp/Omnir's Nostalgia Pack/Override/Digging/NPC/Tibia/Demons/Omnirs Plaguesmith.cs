...
int drownTimer = 5000;
int drowningRisk = 2500;
//Spawns in Hardmode Underworld, mostly under 2/10th and after 8/10th (Length) when there are no Town NPCs (Unless a Blood Moon).
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
	...
	if (Main.hardMode)
    {
        ...
        {
	        if (Main.player[playerID].townNPCs <= 0f && Main.rand.Next(80)==1) return true;
	        else if (Main.player[playerID].townNPCs <= 0f && (x < Main.maxTilesX*0.2f || x > Main.maxTilesX*0.8f) && Main.rand.Next(15)==1) return true;
        ...
        if (customAi1 >= 10f)
        {
		    ...
		    if (Main.rand.Next(50)==1)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    int type = Config.projectileID["Enemy Spell Great Poison Strike Ball"];//44;//0x37; //14;
...
#region Fly Through Walls
		...
		if ((!Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height)))
		{
			if ((npc.collideY) || (npc.collideX))
			{
                ...
                else if ((movedTowards >= 5f) &&  (movedTowards <= 10f))
                {
                    npc.velocity.Y = 0f;
                    npc.velocity.X = 0f;
                    movedTowards += 1;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Plaguesmith Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Plaguesmith Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Plaguesmith Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Plaguesmith Gore 4", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Plaguesmith Gore 5", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Plaguesmith Gore 6", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Plaguesmith Gore 6", 1f, -1);
    }
}
#endregion
