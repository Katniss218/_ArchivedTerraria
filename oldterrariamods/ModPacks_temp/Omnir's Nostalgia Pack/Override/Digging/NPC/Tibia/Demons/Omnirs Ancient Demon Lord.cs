...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (NPC.AnyNPCs(Config.npcDefs.byName["Omnirs Ancient Demon Lord"].type) ||Main.player[playerID].zoneJungle || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
    if (Main.hardMode && Main.bloodMoon && !oSky && Main.rand.Next(1000)==1) return true;
    if (Main.bloodMoon &&  !oSky && Main.rand.Next(3000)==1) return true;
    if (oUnderworld)
    {
        ...
        {
            if (Main.rand.Next(150)==1) return true;
	        else if ((x < Main.maxTilesX*0.25f || x > Main.maxTilesX*0.75f) && Main.rand.Next(50)==1) return true;
	        return false;
        }
        return false;
    }
...
#region Projectiles
        ...
        if (customAi1 >= 10f)
        {
            ...
            if ((customspawn1 < 4) && Main.rand.Next(200)==1)
	        {
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
                ...
                if (Main.rand.Next(500)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
                    {
			            ...
			            speedX *= num51;
			            speedY *= num51;
			            int damage = 250;//(int) (14f * npc.scale);
