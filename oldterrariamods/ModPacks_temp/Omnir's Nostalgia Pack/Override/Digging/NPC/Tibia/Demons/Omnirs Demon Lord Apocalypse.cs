...
//Spawns in the Underworld. Spawns more under 2.5/10th and again after 7.5/10th (Length). Spawns more in Hardmode. Will not spawn if there are more than 2 Town NPCs nearby (or if a Blood Moon).
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (NPC.AnyNPCs(Config.npcDefs.byName["Omnirs Demon Lord Apocalypse"].type) || NPC.AnyNPCs(Config.npcDefs.byName["Omnirs Ancient Demon Lord"].type) || Main.player[playerID].zoneJungle || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
    if (Main.hardMode && Main.bloodMoon && Main.rand.Next(2500)==1) return true;
    if (Main.bloodMoon && y > Main.worldSurface * 0.5f && Main.rand.Next(10000)==1) return true;
    if (oUnderworld)
    {
	    if (Main.player[playerID].townNPCs < 3f)
        {
            if (Main.rand.Next(5000)==1) return true;
	        else if ((x < Main.maxTilesX*0.25f || x > Main.maxTilesX*0.75f) && Main.rand.Next(2500)==1) return true;
	        return false;
        }
        if (Main.hardMode && Main.player[playerID].townNPCs < 3f)
        {
            if (Main.rand.Next(200)==1) return true;
	        else if ((x < Main.maxTilesX*0.25f || x > Main.maxTilesX*0.75f) && Main.rand.Next(100)==1) return true;
	        return false;
        }
        return false;
    }
...
#region Projectiles
        ...
        if (customAi1 >= 10f)
        {
            ...
            if ((customspawn1 < 4) && Main.rand.Next(2)==1)
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Omnirs Ancient Demon", 0);
		        ...
		        if (Main.netMode == 2)
		        {
		            NetMessage.SendData(23, -1, -1, "Ancient Demon", Spawned, 0f, 0f, 0f, 0);
		        }
	        }
		    ...
		    if (Main.rand.Next(200)==1)
            {
			    ...
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
				    ...
				    int type = Config.projectileID["Enemy Spell Great Energy Beam Ball"];//44;//0x37; //14;
				    int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
				    Main.projectile[num54].timeLeft = 0;
				    Main.projectile[num54].aiStyle=1;
                        Main.PlaySound(2, (int) npc.position.X, (int) npc.position.Y, 0x11);
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
			    ...
			    if (Main.rand.Next(100)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        speedY *= num51;
				        int damage = 180;
				        int type = Config.projectileID["Enemy Great Attack"];//44;//0x37; //14;
				        int num54 = Projectile.NewProjectile(vector8.X, vector8.Y, speedX, speedY, type, damage, 0f, Main.myPlayer);
		...
		if ((!Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height)))
		{
			if ((npc.collideY) || (npc.collideX))
			{
                ...
                if (movedTowards <= 4f) 
                {
				    ...
				    {
					    Vector2 arg_634E_0 = new Vector2(npc.position.X - 20f, npc.position.Y - 20f);
					    int arg_634E_1 = npc.width + 40;
					    int arg_634E_2 = npc.height + 40;
					    int arg_634E_3 = 5;
                ...
                else if ((movedTowards >= 5f) &&  (movedTowards <= 7f))
                {
                    npc.velocity.Y = 0f;
                    npc.velocity.X = 0f;
                    movedTowards += 1;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Demon Lord Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Demon Lord Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Demon Lord Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Demon Lord Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Demon Lord Gore 3", 1f, -1);
    }
}
