...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.hardMode && Main.bloodMoon && !oSky && Main.rand.Next(100)==1) return true;
    if (Main.bloodMoon && !oSky && Main.rand.Next(1000)==1) return true;
    if (oMagmaCavern || oUnderworld)
    {
	    if (Main.player[playerID].townNPCs < 3f)
        {
            if (Main.rand.Next(150)==1) return true;
	        else if ((x < Main.maxTilesX*0.25f || x > Main.maxTilesX*0.75f) && Main.rand.Next(40)==1) return true;
	        return false;
        }
        if (Main.hardMode && Main.player[playerID].townNPCs < 3f)
        {
            if (Main.rand.Next(25)==1) return true;
	        else if ((x < Main.maxTilesX*0.25f || x > Main.maxTilesX*0.75f) && Main.rand.Next(5)==1) return true;
	        return false;
        }
        return false;
    }
...
#region Projectiles
        ...
        if (customAi1 >= 10f)
        {
            ...
            if ((customspawn1 < 1) && Main.rand.Next(2)==1)
	        {
		        int Spawned = NPC.NewNPC((int) npc.position.X+(npc.width/2), (int) npc.position.Y+(npc.height/2), "Omnirs Fiery Fire Elemental", 0);
		        ...
		        if (Main.netMode == 2)
		        {
		            NetMessage.SendData(23, -1, -1, "Fiery Fire Elemental", Spawned, 0f, 0f, 0f, 0);
		        }
	        }
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Demon Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Demon Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Demon Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Demon Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Demon Gore 3", 1f, -1);
    }
}
