...
int num95 = 0;
int noJump = 0;
int drownTimerMax = 3800;
int drownTimer = 3800;
int drowningRisk = 1000;
//Spawns in the Cavern(Normal is only before 2/10th and after 8/10th (Width)), or during a Blood Moon. Does not spawn in Dungeon, Meteor, or if more than 1 Town NPC is nearby.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].townNPCs > 1f || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
    if (!Main.hardMode)
    {
        if (oCavern && (x < Main.maxTilesX*0.2f || x > Main.maxTilesX*0.8f) && Main.rand.Next(150)==1) return true;
	    if (Main.bloodMoon && Main.rand.Next(1000)==1) return true;
        return false;
    }
    if (Main.hardMode)
    {
        if (oCavern && Main.rand.Next(50)==1) return true;
        if (oCavern && (x < Main.maxTilesX*0.2f || x > Main.maxTilesX*0.8f) && Main.rand.Next(5)==1) return true;
	    if (Main.bloodMoon && Main.rand.Next(50)==1) return true;
        return false;
    }
    return false;
}
...
// set parameters
    ...
    float top_speed=2.2f;  //  max walking speed, also affects jump length
    ...
    float enrage_top_speed=4.4f;  //  faster when enraged, usually 2*top_speed
...
#region Fly Through Walls
		...
		if ((!Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height)))
		{
			if ((npc.collideY) || (npc.collideX))
			{
                ...
                else if ((movedTowards >= 5f) &&  (movedTowards <= 12f))
                {
                    npc.velocity.Y = 0f;
                    npc.velocity.X = 0f;
                    movedTowards += 1;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Behemoth Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Behemoth Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Behemoth Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Behemoth Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Behemoth Gore 3", 1f, -1);
    }
}
