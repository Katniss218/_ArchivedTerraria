...
//Spawns in cavern of Normal Mode, most before 2/10th and after 8/10th (Width). Spawns in Hardmode beyond the Underground, mostly between 2/10th to 6/10th and again after 8/10th (Width). Does not spawn in Dungeons, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].townNPCs > 0f || Main.player[playerID].zoneDungeon) return false;
    if (oCavern) 
    {
	    if (Main.rand.Next(800)==1) return true;
	    else if ((x < Main.maxTilesX*0.2f || x > Main.maxTilesX*0.8f) && Main.rand.Next(100)==1) return true;
	    return false;
    }
	if (Main.hardMode && oCavern)
    {
	    if (Main.rand.Next(50)==1) return true;
	    else if ((x > Main.maxTilesX*0.2f && x < Main.maxTilesX*0.6f || x > Main.maxTilesX*0.8f) && Main.rand.Next(20)==1) return true;
	    return false;
    }
    return false;
}
...
#region Fly Through Walls
	    ...
	    if ((!Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height)))
	    {
		    if ((npc.collideY) || (npc.collideX))
		    {
                ...
                else if ((movedTowards >= 5f) &&  (movedTowards <= 35f))
                {
                    npc.velocity.Y = 0f;
                    npc.velocity.X = 0f;
                    movedTowards += 1;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Rock Golem Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Rock Golem Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Rock Golem Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Rock Golem Gore 4", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Rock Golem Gore 5", 1f, -1);
    }
}
#endregion
