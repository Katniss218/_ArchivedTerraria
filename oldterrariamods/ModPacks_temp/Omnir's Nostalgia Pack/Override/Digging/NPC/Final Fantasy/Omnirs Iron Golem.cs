...
//Spawns in Underground Hardmode, mostly before 1.5/10th and after 8.5/10th (Width).
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if (Main.rand.Next(300)==1) return true;
	    else if ((x < Main.maxTilesX*0.15f || x > Main.maxTilesX*0.85f) && Main.rand.Next(50)==1) return true;
	    return false;
    }
    return false;
}
		    ...
		    if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))           
            {
		        if (Main.rand.Next(180)==1)
                {
			        ...
			        if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			        {
				        ...
				        speedY *= num51;
				        int damage = 120;
				        int type = Config.projectileID["Enemy Spell Toxin"];//44;//0x37; //14;
...
#region Fly Through Walls
		...
		if ((!Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height)))
		{
			if ((npc.collideY) || (npc.collideX))
			{
                ...
                else if ((movedTowards >= 5f) &&  (movedTowards <= 23f))
                {
                    npc.velocity.Y = 0f;
                    npc.velocity.X = 0f;
                    movedTowards += 1;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Iron Golem Gore 1", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Iron Golem Gore 2", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Iron Golem Gore 3", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Iron Golem Gore 4", 1.1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Iron Golem Gore 5", 1.1f, -1);
    }
}
#endregion
