...
int num95 = 0;
int noJump = 0;
int drownTimerMax = 500;
int drownTimer = 500;
int drowningRisk = 400;
//Spawns in the lower Cavern and Underworld. Spawns less in Hardmode. Does not spawn in Dungeons, Jungle, Meteor, or if there are more than 2 Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (Main.player[playerID].townNPCs > 2f || Main.player[playerID].zoneJungle || Main.player[playerID].zoneDungeon || Main.player[playerID].zoneMeteor) return false;
	if (!Main.hardMode && (oMagmaCavern || oUnderworld))
    {
	    if (Main.rand.Next(50)==1) return true;
	    if (oUnderworld && Main.rand.Next(30)==1) return true;
        return false;
    }
	if (Main.hardMode && oUnderworld)
    {
	    if (Main.rand.Next(100)==1) return true;
        return false;
    }
    return false;
}
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Gigas Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Gigas Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Gigas Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Gigas Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Fire Gigas Gore 3", 1f, -1);
    }
}
