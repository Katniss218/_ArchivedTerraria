...
//Does not spawn with Town NPCs. Spawns from the surface to just before the Underworld. Spawns most before 3/10ths or after 7/10th of the Underground and Cavern(Width). Does not spawn in Dungeons, or Meteor.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    if (!oUnderworld)
    {
	    if ((oSurface || oUnderSurface || oUnderground) && Main.player[playerID].statLifeMax >300 && Main.rand.Next(2000)==1) return true;
	    else if (oUnderground && (x < Main.maxTilesX*0.3f || x > Main.maxTilesX*0.7f) && Main.rand.Next(800)==1) return true;
        else if (oCavern && (x < Main.maxTilesX*0.3f || x > Main.maxTilesX*0.7f) && Main.rand.Next(30)==1) return true;
	    return false;
    }
    return false;
}
...
#region Fly Through Walls
	    ...
	    if ((!Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height)))
	    {
		    if ((npc.collideY) || (npc.collideX))
		    {
                ...
                else if ((movedTowards >= 5f) &&  (movedTowards <= 29f))
                {
                    npc.velocity.Y = 0f;
                    npc.velocity.X = 0f;
                    movedTowards += 1;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Clay Golem Gore 1", 1.05f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Clay Golem Gore 2", 1.05f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Clay Golem Gore 3", 1.05f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Clay Golem Gore 4", 1.05f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Clay Golem Gore 5", 1.05f, -1);
    }
}
#endregion
