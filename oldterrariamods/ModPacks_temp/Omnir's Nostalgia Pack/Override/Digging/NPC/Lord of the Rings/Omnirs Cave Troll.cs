...
int num95 = 0;
int noJump = 0;
int drownTimerMax = 3200;
int drownTimer = 3200;
int drowningRisk = 600;
//Spawns from the Surface into the Cavern. Spawns a lot on Blood Moons beyond 6.5/10th (Width). Does not spawn in Hardmode, Dungeons, Meteor, Jungle, or if there are Town NPCs.
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
	    if (oCavern && Main.rand.Next(150)==1) return true;
	    else if (x > Main.maxTilesX*0.65f && Main.bloodMoon && Main.rand.Next(20)==1) return true;
	    return false;
    }
    return false;
}
...
// set parameters
    ...
    float enrage_top_speed=1.5f;  //  faster when enraged, usually 2*top_speed
...
#region Fly Through Walls
		...
		if ((!Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height)))
		{
            ...
            else if ((movedTowards >= 5f) &&  (movedTowards <= 50f))
                {
                    npc.velocity.Y = 0f;
                    npc.velocity.X = 0f;
                    movedTowards += 1;
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Cave Troll Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Cave Troll Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Cave Troll Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Cave Troll Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Cave Troll Gore 3", 1f, -1);
    }
}
