float customAi1;
int drownTimerMax = 6000;
int drownTimer = 6000;
int drowningRisk = 2000;
//Spawns in the Cavern and Magma Cavern, before 2/10ths and after 8/10ths (Width), or in the Corruption(Hardmode). Does not spawn in the Dungeons, Meteor, or if there are more than 2 Town NPCs (Except on Blood Moons).
...
public static bool SpawnNPC(int x, int y, int playerID) 
{
    ...
    {
        if ((oCavern && x < Main.maxTilesX*0.2f || oCavern && x > Main.maxTilesX*0.8f) && Main.rand.Next(600)==1) return true;
        else if (Main.hardMode && Main.rand.Next(200)==1) return true;
    	return false;
    }
    else if (Main.hardMode && Main.player[playerID].zoneEvil && Main.rand.Next(400)==1) return true;
    else if (Main.bloodMoon && Main.rand.Next(800)==1) return true;
    return false;
}
#endregion
...
#region Bricksmash
		if ((!Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height)) && (((!Main.hardMode) && npc.position.Y < (Main.rockLayer * 35f)) || (npc.justHit) || (Main.hardMode)))
		{
            ...
            {
			    float num48 = 8f;
			    Vector2 vector8 = new Vector2((npc.position.X + ((npc.width * 0.25f) * (npc.direction*2))/20f)+130, npc.position.Y + (npc.height));
			    float speedX = ((Main.player[npc.target].position.X + (Main.player[npc.target].width * 0.5f)) - vector8.X) + Main.rand.Next(-20, 0x15);
			    float speedY = ((Main.player[npc.target].position.Y + (Main.player[npc.target].height * 0.5f)) - vector8.Y) + Main.rand.Next(-20, 0x15);
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
            ...
            {
			    float num48 = 8f;
			    Vector2 vector8 = new Vector2((npc.position.X + ((npc.width * 0.25f) * (npc.direction*2))/20f)+130, npc.position.Y + (npc.height/2));
			    float speedX = ((Main.player[npc.target].position.X + (Main.player[npc.target].width * 0.5f)) - vector8.X) + Main.rand.Next(-20, 0x15);
			    float speedY = ((Main.player[npc.target].position.Y + (Main.player[npc.target].height * 0.5f)) - vector8.Y) + Main.rand.Next(-20, 0x15);
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
            ...
            {
			    float num48 = 8f;
			    Vector2 vector8 = new Vector2((npc.position.X + ((npc.width * 0.25f) * (npc.direction*2))/20f)+130, npc.position.Y + (npc.height/4));
			    float speedX = ((Main.player[npc.target].position.X + (Main.player[npc.target].width * 0.5f)) - vector8.X) + Main.rand.Next(-20, 0x15);
			    float speedY = ((Main.player[npc.target].position.Y + (Main.player[npc.target].height * 0.5f)) - vector8.Y) + Main.rand.Next(-20, 0x15);
			    if (((speedX < 0f) && (npc.velocity.X < 0f)) || ((speedX > 0f) && (npc.velocity.X > 0f)))
			    {
...
public void NPCLoot()
{
    if (npc.life <= 0)
    {
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Great Dragon Gore 1", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Great Dragon Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Great Dragon Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Great Dragon Gore 2", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Great Dragon Gore 3", 1f, -1);
        Gore.NewGore(npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), "Ancient Great Dragon Gore 4", 1f, -1);
    }
}
#endregion 
