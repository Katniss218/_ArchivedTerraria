...
public void Kill()
{
	...
	projectile.timeLeft = 0;
    {
        ...
        if (projectile.position.Y + (float)(projectile.height / 2) > Main.player[projectile.owner].position.Y + (float)(Main.player[projectile.owner].height / 2))
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width/2), projectile.position.Y + (float)(projectile.height/2), 0, 0, "Spell Quake 4", (int) ((160 + (Main.player[projectile.owner].statMana/10)) * (Main.player[projectile.owner].magicDamage)), 0f, projectile.owner);
        }
