...
public void Kill()
{
	...
	projectile.timeLeft = 0;
    {
        ...
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*-9), projectile.position.Y + (float)(projectile.height-0.5f), 0, 0, "Enemy Spell Energy Beam", 25, 8f, projectile.owner);
        }
        else
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*10f), projectile.position.Y + (float)(projectile.height-0.5f), 0, 0, "Enemy Spell Energy Beam", 25, 8f, projectile.owner);
        }
