...
public void Kill()
{
	...
	projectile.timeLeft = 0;
    {
        ...
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*-12), projectile.position.Y + (float)(projectile.height*-0.3f), 0, 0, "Enemy Spell Great Fire Wave Reverse", 75, 3f, projectile.owner);
        }
        else
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*11.4f), projectile.position.Y + (float)(projectile.height*-0.3f), 0, 0, "Enemy Spell Great Fire Wave", 75, 3f, projectile.owner);
        }
