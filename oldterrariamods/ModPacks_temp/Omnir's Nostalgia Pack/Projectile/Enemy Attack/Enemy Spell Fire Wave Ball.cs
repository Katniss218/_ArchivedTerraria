...
public void Kill()
{
	...
	projectile.timeLeft = 0;
    {
        ...
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*-8), projectile.position.Y + (float)(projectile.height*-0.3f), 0, 0, "Enemy Spell Fire Wave Reverse", 23, 3f, projectile.owner);
        }
        else
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*7.4f), projectile.position.Y + (float)(projectile.height*-0.3f), 0, 0, "Enemy Spell Fire Wave", 23, 3f, projectile.owner);
        }
