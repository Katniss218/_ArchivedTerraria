...
public void Kill()
{
	...
	projectile.timeLeft = 0;
    {
        ...
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*-8.5f), projectile.position.Y + (float)(projectile.height), 0, 0, "Enemy Spell Energy Wave Reverse", 55, 8f, projectile.owner);
        }
        else
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*9.2f), projectile.position.Y + (float)(projectile.height), 0, 0, "Enemy Spell Energy Wave", 55, 8f, projectile.owner);
        }
