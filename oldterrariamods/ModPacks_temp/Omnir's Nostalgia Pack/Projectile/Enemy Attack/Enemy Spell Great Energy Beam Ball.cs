...
public void Kill()
{
	...
	projectile.timeLeft = 0;
    {
        ...
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*-13), projectile.position.Y + (float)(projectile.height-0.5f), 0, 0, "Enemy Spell Great Energy Beam", 105, 8f, projectile.owner);
        }            
        else
        {
            if(projectile.owner == Main.myPlayer) Projectile.NewProjectile(projectile.position.X + (float)(projectile.width*14f), projectile.position.Y + (float)(projectile.height-0.5f), 0, 0, "Enemy Spell Great Energy Beam", 105, 8f, projectile.owner);
        }
